...
TStr TSAppSrvFun::GetFldVal(const TStrKdV& FldNmValPrV, const TStr& FldNm, const TStr& DefFldVal)
{
...
uint64 TSAppSrvFun::GetFldUInt64(const TStrKdV& FldNmValPrV, const TStr& FldNm,
                                 const uint64& DefVal)
{
...
bool TSAppSrvFun::IsFldNmVal(const TStrKdV& FldNmValPrV, const TStr& FldNm, const TStr& FldVal)
{

    int ValN = FldNmValPrV.SearchForw(TStrKd(FldNm, ""));
    while (ValN != -1) {
        if (FldNmValPrV[ValN].Dat == FldVal) {
            return true;
        }
        ValN = FldNmValPrV.SearchForw(TStrKd(FldNm, ""), ValN + 1);
    }
...
void TSAppSrvFun::Exec(const TStrKdV& FldNmValPrV, const PSAppSrvRqEnv& RqEnv)
{
    ...
    try {
        ...
        if (NotifyOnRequest)
            Notify->OnStatus(TStr::Fmt("RequestFinish %s [request took %d ms]", FunNm.CStr(),
                                       StopWatch.GetMSecInt()));
        // prepare response
        HttpResp = THttpResp::New(THttp::OkStatusCd, ContTypeVal, false, BodySIn, TStr(),
                                  StopWatch.GetMSecInt());
    }
        ...
        // prepare response
        HttpResp =
            THttpResp::New(THttp::InternalErrStatusCd, ContTypeVal, false, TMIn::New(ResStr));
    }
    catch (...) {
...
THttpReqSerInfo::THttpReqSerInfo(const TStr& _UrlRel, const TStr& _UrlBase,
                                 const THttpRqMethod& _ReqMethod, const TMem& _Body)
...
TSAppSrv::TSAppSrv(const int& PortN, const TSAppSrvFunV& SrvFunV, const PNotify& Notify,
                   const bool& _ShowParamP, const bool& _ListFunP)
    : TWebSrv(PortN, true, Notify), Favicon(Favicon_bf, Favicon_len)
{
...
void TSAppSrv::OnHttpRq(const uint64& SockId, const PHttpRq& HttpRq)
{
    ...
    try {
        ...
        if (FunNm == "favicon.ico") {
            PHttpResp HttpResp =
                THttpResp::New(THttp::OkStatusCd, THttp::ImageIcoFldVal, false, Favicon.GetSIn());
        ...
        TStrKdV FldNmValPrV;
        HttpRqUrlEnv->GetKeyValPrV(FldNmValPrV);

        // report call
        if (ShowParamP) {
            GetNotify()->OnStatus(HttpRq->GetUrl()->GetUrlStr());
        }
        ...
        if (!FunNm.Empty()) {
            ...
            // prepare response
            PHttpResp HttpResp =
                THttpResp::New(THttp::OkStatusCd, THttp::AppJSonFldVal, false, TMIn::New(ResStr));
            // send response
            SendHttpResp(SockId, HttpResp);
        }
    }
        ...
        TNotify::StdNotify->OnNotifyFmt(ntErr, "Error location info: %s",
                                        Except->GetLocStr().CStr());
        ...
        // prepare response
        PHttpResp HttpResp =
            THttpResp::New(ErrStatusCd, THttp::AppJSonFldVal, false, TMIn::New(ResStr));
        // send response
        SendHttpResp(SockId, HttpResp);
    }
    catch (...) {
...
TReplaySrv::TReplaySrv(const int& PortN, const TSAppSrvFunV& SrvFunV, const PNotify& Notify,
                       const bool& _ShowParamP, const bool& _ListFunP)
    : TSAppSrv(PortN, SrvFunV, Notify, _ShowParamP, _ListFunP)
{
}
...
bool TReplaySrv::ReplayStream(const PSIn& SIn, const PNotify& ErrorNotify)
{
    ...
    while (!SIn->Eof()) {
        try {
            ...
            ErrorNotify->OnNotifyFmt(
                ntErr, "TReplaySrv::ReplayStream. Exception while loading next request: %s",
                E->GetMsgStr().CStr());
        }
        catch (...) {
            ErrorNotify->OnNotifyFmt(
                ntErr, "TReplaySrv::ReplayStream. General exception while loading next request.");
        }
    }
    return true;
}
...
// by setting Append = false you're effectively clearing the file first before starting to write in
// it
...
bool TReplaySrv::RemoveLogData(const TStr& LogFNm)
{
    try {
        ...
        TNotify::StdNotify->OnNotifyFmt(
            ntErr, "TReplaySrv::RemoveLogData. Error while removing file: %s\n%s.", LogFNm.CStr(),
            Except->GetLocStr().CStr());
    }
    catch (...) {
        TNotify::StdNotify->OnNotifyFmt(
            ntErr, "TReplaySrv::RemoveLogData. Error while removing file: %s.", LogFNm.CStr());
    }
    return false;
}
...
void TReplaySrv::OnHttpRq(const uint64& SockId, const PHttpRq& HttpRq)
{
    ...
    if (!SOut.Empty()) {
        try {
            ...
            Notify.OnNotifyFmt(ntErr, "TReplaySrv::OnHttpRq. Exception %s.",
    ...
    // only after we've logged the request we also execute it - otherwise we could process the
    // request and crash before saving the data
...
PSIn TSASFunFPath::ExecSIn(const TStrKdV& FldNmValPrV, const PSAppSrvRqEnv& RqEnv,
                           TStr& ContTypeStr)
{
    ...
    if (FExt == ".htm") {
        ...
        ContTypeStr = THttp::TextJavaScriptFldVal;
    }
    else if (FExt == ".css") {
        ContTypeStr = THttp::TextCssFldVal;
    }
    else if (FExt == ".ico") {
        ContTypeStr = THttp::ImageIcoFldVal;
    }
    else if (FExt == ".png") {
        ContTypeStr = THttp::ImagePngFldVal;
    }
    else if (FExt == ".jpg") {
        ContTypeStr = THttp::ImageJpgFldVal;
    }
    else if (FExt == ".jpeg") {
        ContTypeStr = THttp::ImageJpgFldVal;
    }
    else if (FExt == ".gif") {
        ContTypeStr = THttp::ImageGifFldVal;
    }
    else {
...
TSASFunRedirect::TSASFunRedirect(const TStr& FunNm, const TStr& SettingFNm)
    : TSAppSrvFun(FunNm, saotUndef)
{
    ...
    while (FIn.GetNextLn(LnStr)) {
        TStrV PartV;
        LnStr.SplitOnAllCh('\t', PartV, false);
        if (PartV.Empty()) {
            continue;
        }
        if (PartV[0].Empty()) {
