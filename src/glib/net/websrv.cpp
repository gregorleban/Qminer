...
void TWebSrvSockEvent::OnError(const uint64& SockId, const int& ErrCd, const TStr& ErrStr)
{
...
/////////////////////////////////////////////////
// Web-Server
TWebSrv::TWebSrv(const int& _PortN, const bool& FixedPortNP, const PNotify& _Notify)
    : Notify(_Notify), PortN(_PortN), HomeNrFPath(TStr::GetNrFPath(TDir::GetCurDir())), SockEvent(),
      Sock(), SockIdToConnH()
{
...
void TWebSrv::OnHttpRq(const uint64& SockId, const PHttpRq& HttpRq)
{
    ...
    if (!RqUrl->GetPathStr().Empty()) {
        ...
        if (!RqFOpened) {
            ...
            HttpResp = PHttpResp(new THttpResp(THttp::ErrNotFoundStatusCd, THttp::TextHtmlFldVal,
                                               false, BodySIn, ""));
        }
        else {
            ...
            if (THttp::IsHtmlFExt(RqFNm.GetFExt())) {
                ...
                HttpResp = PHttpResp(
                    new THttpResp(THttp::OkStatusCd, THttp::TextHtmlFldVal, false, BodySIn, ""));
            }
            else if (THttp::IsGifFExt(RqFNm.GetFExt())) {
                // send image/gif mime type if Gif file
                HttpResp = PHttpResp(
                    new THttpResp(THttp::OkStatusCd, THttp::ImageGifFldVal, false, BodySIn, ""));
            }
            else {
                // send application/octet mime type
                HttpResp = PHttpResp(
                    new THttpResp(THttp::OkStatusCd, THttp::AppOctetFldVal, false, BodySIn, ""));
            }
        }
    }
    else {
        ...
        HtmlChA += "</body></html>";
        PSIn BodySIn = TMIn::New(HtmlChA);
        HttpResp = THttpResp::New(THttp::OkStatusCd, THttp::TextHtmlFldVal, false, BodySIn);
    }
