...
class TWebPgFetchEvent : public TSockEvent {
  private:
    ...
    {
        KeyNm = CookieKeyValDmPathQuV[CookieN].Val1;
        ValStr = CookieKeyValDmPathQuV[CookieN].Val2;
        DmNm = CookieKeyValDmPathQuV[CookieN].Val3;
        PathStr = CookieKeyValDmPathQuV[CookieN].Val4;
    }
  ...
  public:
    TWebPgFetchEvent(TWebPgFetch* _Fetch, const int& _FId, const PUrl& _Url, const TStr& _ProxyStr,
                     const TStr& _UserAgentStr, const int& _TimeOutMSecs);
    ~TWebPgFetchEvent() {}

    TSockEvent& operator=(const TSockEvent&)
    {
        Fail;
        return *this;
    }
    ...
    {
        if ((StartMSecs == TUInt64::Mx) || (EndMSecs == TUInt64::Mx)) {
            return -1;
        }
        else {
            return int(EndMSecs - StartMSecs);
        }
    }
    int GetBytesRead() const { return SockMem.Len(); }
    PUrl GetUrl() const { return TopUrl; }
};
...
TWebPgFetchEvent::TWebPgFetchEvent(TWebPgFetch* _Fetch, const int& _FId, const PUrl& _Url,
                                   const TStr& _ProxyStr, const TStr& _UserAgentStr,
                                   const int& _TimeOutMSecs)
    : TSockEvent(), Fetch(_Fetch), FId(_FId), TopUrl(_Url), ProxyStr(_ProxyStr),
      UserAgentStr(_UserAgentStr), CurUrl(_Url), UrlStrV(), SockHost(NULL), Sock(NULL), Retries(0),
      TimeOutMSecs(_TimeOutMSecs), OppSockClosed(true), SockMem(),
      StartMSecs(TTm::GetCurUniMSecs()), EndMSecs(TUInt64::Mx)
{
}
...
void TWebPgFetchEvent::OnFetchEnd(const PHttpResp& HttpResp)
{
    ...
    if (StatusCd / 100 == 2) { // codes 2XX - ok
    ...
    else if (StatusCd / 100 == 3) { // codes 3XX - redirection
        ChangeLastUrlToLc(HttpResp);
        if (UrlStrV.Len() < 5) {
            ...
            if (RedirUrl->IsOk(usHttp)) {
                TStrQuV CookieQuV;
                HttpResp->GetCookieKeyValDmPathQuV(CookieQuV);
        ...
        TStr MsgStr =
            TStr("Http Error (") + TInt::GetStr(StatusCd) + "/" + HttpResp->GetReasonPhrase() + ")";
        OnFetchError(MsgStr);
    }
}
...
{
    PSockEvent Event =
        PSockEvent(new TWebPgFetchEvent(this, FId, Url, ProxyStr, UserAgentStr, TimeOutMSecs));
    TSockEvent::Reg(Event);
    ConnFIdToEventH.AddDat(FId, Event);
    TStr HostNm;
    if (ProxyStr.Empty()) {
        HostNm = Url->GetIpNumOrHostNm();
    }
    else {
        TStr PortNStr;
        ProxyStr.SplitOnCh(HostNm, ':', PortNStr);
    }
...
{
    TIntV ConnFIdV;
    ConnFIdToEventH.GetKeyV(ConnFIdV);
...
/////////////////////////////////////////////////
// Web-Fetch-Blocking
void TWebFetchBlocking::GetWebPg(const PUrl& Url, bool& Ok, TStr& MsgStr, PWebPg& WebPg,
                                 ...
                                 const TStr& UserAgentStr)
{
    ...
    if (!UserAgentStr.Empty()) {
        WebFetch.PutUserAgentStr(UserAgentStr);
    }
    ...
    TLoop::Run();
    Ok = WebFetch.Ok;
    MsgStr = WebFetch.MsgStr;
    WebPg = WebFetch.WebPg;
    if (!Ok) {
        TNotify::OnNotify(Notify, ntInfo, TStr("Fetching Error: [" + MsgStr + "]"));
    }
}

void TWebFetchBlocking::GetWebPg(const TStr& UrlStr, bool& Ok, TStr& MsgStr, PWebPg& WebPg,
                                 const PNotify& Notify, const TStr& ProxyStr,
                                 const TStr& UserAgentStr)
{
...
void TWebFetchBlocking::GetWebPg(const PHttpRq& HttpRq, bool& Ok, TStr& MsgStr, PWebPg& WebPg,
                                 const PNotify& Notify, const TStr& ProxyStr,
                                 const TStr& UserAgentStr)
{
...
void TWebPgToFileFetch::OnFetch(const int&, const PWebPg& WebPg)
{
    ...
    if (!OutXmlFNm.Empty()) {
        ...
        THtmlDoc::SaveHtmlToXml(HtmlStr, OutXmlFNm, WebPg->GetUrlStr(), OutXmlTextP, OutXmlUrlP,
                                OutXmlToksP, OutXmlTagsP, OutXmlArgsP);
    }
    // save web-page as text
    if (!OutTxtFNm.Empty()) {
        TStr HtmlStr = WebPg->GetHttpBodyAsStr();
        THtmlDoc::SaveHtmlToTxt(HtmlStr, OutTxtFNm, WebPg->GetUrlStr(), OutTxtUrlP, OutXmlTagsP);
    }
    // output to screen
    if (OutScrP) {
...
void TWebPgToFileFetch::OnError(const int&, const TStr& MsgStr)
{
    ...
    if (!OutHttpBodyFNm.Empty()) {
        TFOut SOut(OutHttpBodyFNm);
        TFileId SOutFId = SOut.GetFileId();
        fprintf(SOutFId, "Error: %s\n", MsgStr.CStr());
    }
    // save error-message as http
    if (!OutHttpFNm.Empty()) {
        TFOut SOut(OutHttpFNm);
        TFileId SOutFId = SOut.GetFileId();
        fprintf(SOutFId, "Error: %s\n", MsgStr.CStr());
    }
...
void TWebFetchSendBatchJson::Send()
{
    ...
    TMem BodyMem;
    BodyMem.AddBf(BodyStr.CStr(), BodyStr.Len());
...
/////////////////////////////////////////////////
// TWebPgFetchPersist
TWebPgFetchPersist::TWebPgFetchPersist(const TStr& _SaveFName, const bool& _RepeatFailedRequests,
                                       const bool& _ReportState, const TStr& _ReportPrefix,
                                       const PNotify& _Notify)
    : SaveFName(_SaveFName), RepeatFailedRequests(_RepeatFailedRequests), ReportState(_ReportState),
      ReportPrefix(_ReportPrefix), Notify(_Notify)
{
    PutMxConns(10);
    ...
    if (SaveFName != "" && TFile::Exists(SaveFName)) {
        ...
        if (FileLen > 0)
            TNotify::StdNotify->OnStatusFmt("Loading %.2f MB of pending web requests...",
                                            FileLen / (double)TInt::Mega);
        Load(FIn);
        if (FileLen > 0)
...
void TWebPgFetchPersist::Load(TSIn& SIn)
{
    ...
    while (!SIn.Eof()) {
        try {
            ...
            Notify->OnStatusFmt("TWebPgFetchPersist.Load. Exception while loading url: %s",
                                ex->GetMsgStr().CStr());
        }
        catch (...) {
            Notify->OnStatus(
                "TWebPgFetchPersist.Load. Unrecognized exception while loading a url.");
        }
    }
    Notify->OnStatusFmt(
        "TWebPgFetchPersist.Load. Loaded %d pending outgoing web requests from disk", Count);
}
...
void TWebPgFetchPersist::Save(TSOut& SOut) const
{
    ...
    TNotify::StdNotify->OnStatusFmt(
        "TWebPgFetchPersist.Save. Saved %d pending outgoing web requests to the disk.",
        WaitFIdUrlPrL.Len() + ConnFIdToEventH.Len());
}
...
void TWebPgFetchPersist::OnError(const int& FId, const TStr& MsgStr)
{
    ...
    if (!Notify.Empty()) {
    TStr Error = "TWebPgFetchPersist.OnError: Received http response 400 (Bad request). Skipping the
    request. Request: " + Url; Notify->OnStatus(Error.CStr());
    }
    return;
    }
    else if (!HttpResp.Empty() && !Notify.Empty()) {
    Notify->OnStatusFmt("TWebPgFetchPersist.OnError: Received http response %d.",
    HttpResp->GetStatusCd());
    }*/
...
{
    if (ReportState)
        printf("%squeue size: %6d. Completed: %6d. Failed: %6d\r", ReportPrefix.CStr(),
               WaitFIdUrlPrL.Len(), SuccessCount.Val, ErrorCount.Val);
}
