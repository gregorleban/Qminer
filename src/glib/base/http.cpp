...
TStr THttp::GetReasonPhrase(const int& StatusCd)
{
    ...
    default:
        return TStr("Unknown Status Code ") + TInt::GetStr(StatusCd);
    }
}
...
{
    IAssert(ChTyV[Ch - TCh::Mn] == int(hpctUndef));
    ChTyV[Ch - TCh::Mn] = TInt(ChTy);
}
...
void THttpChDef::SetLcCh(const TStr& Str)
{
    for (int ChN = 1; ChN < Str.Len(); ChN++) {
        LcChV[Str[ChN] - TCh::Mn] = TCh(Str[0]);
    }
}

THttpChDef::THttpChDef() : ChTyV(TCh::Vals), LcChV(TCh::Vals)
{
...
class THttpLx {
  ...
  public:
    THttpLx(const PSIn& _SIn) : SIn(_SIn), EofChPrS(), Ch(' '), AtEof(false), SfMem(50000)
    {
        GetFirstCh();
    }
    ...
    {
        EofChPrS.Push(TBoolChPr(AtEof, Ch));
        Ch = _Ch;
        AtEof = false;
        SfMem.Pop();
    }
...
char THttpLx::GetCh()
{
    if (EofChPrS.Empty()) {
        ...
        SfMem += Ch;
        AtEof = EofChPrS.Top().Val1;
        Ch = EofChPrS.Top().Val2;
        EofChPrS.Pop();
        return Ch;
    }
}
...
THttpRqMethod THttpLx::GetRqMethod()
{
    ...
    if (MethodNm == THttp::GetMethodNm) {
        Method = hrmGet;
    }
    else if (MethodNm == THttp::HeadMethodNm) {
        Method = hrmHead;
    }
    else if (MethodNm == THttp::PostMethodNm) {
        Method = hrmPost;
    }
    else if (MethodNm == THttp::PutMethodNm) {
        Method = hrmPut;
    }
    else if (MethodNm == THttp::PatchMethodNm) {
        Method = hrmPatch;
    }
    else if (MethodNm == THttp::OptionsMethodNm) {
        Method = hrmOptions;
    }
    else if (MethodNm == THttp::DeleteMethodNm) {
        Method = hrmDelete;
    }
    if (Method == hrmUndef) {
        throw THttpEx(heMethodNmExpected);
    }
    return Method;
}
...
bool THttpLx::IsRespStatusLn()
{
    ...
    }
    {
        for (int ChN = 0; ChN < MouldChA.Len(); ChN++) {
            if (MouldChA[ChN] == 'N') {
                if (!ChDef.IsDigit(TestChA[ChN])) {
                    return false;
                }
            }
            else {
                if (MouldChA[ChN] != TestChA[ChN]) {
                    return false;
                }
            }
        }
...
void THttpLx::GetPeriod()
{
    GetWs();
    if (Ch != '.') {
        throw THttpEx(hePeriodExpected);
    }
    GetCh();
}
...
TStr THttpLx::GetSpec(const TStr& ExpectStr)
{
    GetLws();
    if (!ChDef.IsSpec(Ch)) {
        throw THttpEx(heTSpecExpected);
    }
    TStr SpecStr(Ch);
    GetCh();
    if (!ExpectStr.Empty()) {
        if (ExpectStr != SpecStr) {
            throw THttpEx(heInvalidTSpec);
        }
    }
    return SpecStr;
}
...
int THttpLx::GetInt(const int& RqPlaces)
{
    ...
    if (RqPlaces != -1) {
        if (CurPlaces != RqPlaces) {
            throw THttpEx(heInvalidNumPlaces);
        }
    }
    return Int;
}
        ...
        while (!ChRet.Eof()) {
            ...
            while ((Ch = ChRet.GetCh()) != '=') {
                ...
                case '%': {
                    ...
                    if (Ch1 == 0 && Ch2 == 0)
                        throw THttpEx(heBadSearchStr);
                    KeyNm.AddCh(char(16 * TCh::GetHex(Ch1) + TCh::GetHex(Ch2)));
                } break;
                case '+':
                    KeyNm.AddCh(' ');
                    break;
            ...
            while ((!ChRet.Eof()) && ((Ch = ChRet.GetCh()) != '&')) {
                ...
                case '%': {
                    ...
                    if (Ch1 == 0 && Ch2 == 0)
                        throw THttpEx(heBadSearchStr);
                    ValStr.AddCh(char(16 * TCh::GetHex(Ch1) + TCh::GetHex(Ch2)));
                } break;
                case '+':
                    ValStr.AddCh(' ');
                    break;
...
void THttpRq::ParseHttpRq(const PSIn& SIn)
{
    ...
    Lx.GetWs();
    Lx.GetToken(THttp::HttpStr);
    Lx.GetSpec(THttp::SlashStr);
    MajorVerN = Lx.GetInt(1);
    Lx.GetPeriod();
    MinorVerN = Lx.GetInt(1);
    Lx.GetCrLf();
    // header fields & values
    while ((!Lx.Eof()) && (!Lx.IsCrLf())) {
        TStr FldNm = Lx.GetToken();
        Lx.GetSpec(THttp::ColonStr);
    ...
    if (Method == hrmGet) {
        ParseSearch(Url->GetSearchStr());
             ...
             ((!IsFldNm(THttp::ContTypeFldNm)) ||
              (GetFldVal(THttp::ContTypeFldNm).SearchStr(THttp::TextHtmlFldVal) >=
               0) || // content-type can also contain "charset=utf-8"
    ...
    // if json object then parse the object values
             ...
             GetFldVal(THttp::ContTypeFldNm).SearchStr(THttp::AppJSonFldVal) >= 0) {
        bool Ok;
        TStr MsgStr;
        ...
        if (Ok && Json->IsObj()) {
            for (int N = 0; N < Json->GetObjKeys(); N++) {
                ...
                if (ValJson->IsStr())
                    UrlEnv->AddToKeyVal(Key,
                                        ValJson->GetStr()); // don't put strings into additional ""
                else if (ValJson->IsArr()) {
                    for (int K = 0; K < ValJson->GetArrVals(); K++) {
                        if (ValJson->GetArrVal(K)->IsStr())
                            UrlEnv->AddToKeyVal(
                                Key, ValJson->GetArrVal(K)
                                         ->GetStr()); // don't put strings into additional ""
                        else
                            UrlEnv->AddToKeyVal(Key, ValJson->GetArrVal(K)->SaveStr());
                    }
                }
                else
                    UrlEnv->AddToKeyVal(Key, ValJson->SaveStr());
            }
        }
    }
...
THttpRq::THttpRq(const PSIn& SIn)
    : Ok(false), MajorVerN(0), MinorVerN(0), Method(hrmUndef), FldNmToValH(),
      UrlEnv(TUrlEnv::New()), HdStr(), BodyMem()
{
    try {
        ParseHttpRq(SIn);
    }
...
THttpRq::THttpRq(const THttpRqMethod& _Method, const PUrl& _Url, const TStr& ContTypeFldVal,
                 const TMem& _BodyMem, const int& FetchId)
    : Ok(false), MajorVerN(1), MinorVerN(0), Method(_Method), Url(_Url), FldNmToValH(),
      UrlEnv(TUrlEnv::New()), HdStr(), BodyMem(_BodyMem)
{
    ...
    if (!ContTypeFldVal.Empty()) {
        ...
        HdChA += THttpLx::GetNrStr(THttp::ContTypeFldNm);
        HdChA += ": ";
        HdChA += ContTypeFldVal;
        HdChA += "\r\n";
    }
    // add host
    if (Url->IsOk()) {
        ...
        HdChA += THttpLx::GetNrStr(THttp::HostFldNm);
        HdChA += ": ";
        HdChA += HostNm;
        HdChA += "\r\n";
        ParseSearch(Url->GetSearchStr());
    ...
    if (Url->IsOk() && (FetchId != -1)) {
        ...
        HdChA += THttpLx::GetNrStr(THttp::FetchIdFldNm);
        HdChA += ": ";
        HdChA += FetchIdStr;
        HdChA += "\r\n";
    }
...
const TStr& THttpRq::GetMethodNm() const
{
    switch (Method) {
    case hrmGet:
        return THttp::GetMethodNm;
    case hrmHead:
        return THttp::HeadMethodNm;
    case hrmPost:
        return THttp::PostMethodNm;
    case hrmPut:
        return THttp::PutMethodNm;
    case hrmPatch:
        return THttp::PatchMethodNm;
    case hrmOptions:
        return THttp::OptionsMethodNm;
    case hrmDelete:
        return THttp::DeleteMethodNm;
    default:
...
TStr THttpRq::GetStr() const
{
    ...
    for (int FldN = 0; FldN < FldNmToValH.Len(); FldN++) {
        ChA += FldNmToValH.GetKey(FldN);
        ChA += ": ";
        ChA += FldNmToValH[FldN];
        ChA += "\r\n";
    }
    if (!BodyMem.Empty()) {
        ChA += THttp::ContLenFldNm;
        ChA += ": ";
        ChA += TInt::GetStr(BodyMem.Len());
        ChA += "\r\n";
    }
    ChA += "\r\n";
...
THttpResp::THttpResp(const int& _StatusCd, const TStr& ContTypeVal, const bool& CacheCtrlP,
                     const PSIn& BodySIn, const TStr LocStr, const int& ResponseTimeMs,
                     const TStrKdV& CustomHdrV)
    : Ok(true), MajorVerN(1), MinorVerN(0), StatusCd(_StatusCd), ReasonPhrase(), FldNmToValVH(20),
      HdStr(), BodyMem()
{
    ...
    // first line
    HdChA += "HTTP/";
    HdChA += TInt::GetStr(MajorVerN);
    HdChA += ".";
    HdChA += TInt::GetStr(MinorVerN);
    HdChA += ' ';
    HdChA += TInt::GetStr(StatusCd);
    HdChA += ' ';
    HdChA += ReasonPhrase;
    HdChA += "\r\n";
    // header fields
...
THttpResp::THttpResp(const PSIn& SIn)
    : Ok(false), MajorVerN(0), MinorVerN(0), StatusCd(-1), ReasonPhrase(), FldNmToValVH(20),
      HdStr(), BodyMem()
{
    try {
        ParseHttpResp(SIn);
...
void THttpResp::GetCookieKeyValDmPathQuV(TStrQuV& CookieKeyValDmPathQuV)
{
    ...
    TStrV CookieFldValV;
    GetFldValV(THttp::SetCookieFldNm, CookieFldValV);
    for (int CookieN = 0; CookieN < CookieFldValV.Len(); CookieN++) {
        ...
        for (int KeyValStrN = 0; KeyValStrN < KeyValStrV.Len(); KeyValStrN++) {
            ...
            if (KeyValStr.IsChIn('=')) {
                ...
                KeyNm = KeyNm.GetTrunc();
                ValStr = ValStr.GetTrunc();
            }
            else {
