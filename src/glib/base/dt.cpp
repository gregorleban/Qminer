...
double TRnd::GetBinomialDev(const double &Prb, const int &Trials)
{
    ...
    if (Trials < 25) {
        ...
        do {
            ...
            em = floor(em);
            t = 1.2 * sq * (1.0 + y * y) *
                exp(oldg - (em + 1.0) - TSpecFunc::LnGamma(en - em + 1.0) + em * plog +
                    (en - em) * pclog);
        } while (GetUniDev() > t);
        bnl = em;
    }
...
/////////////////////////////////////////////////
/// Thin Input-Memory
TThinMIn::TThinMIn() : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0) {}

TThinMIn::TThinMIn(const TMemBase &Mem) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0)
{
...
TThinMIn::TThinMIn(const void *_Bf, const int &_BfL) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(_BfL)
{

    Bf = (uchar *)_Bf;
}

TThinMIn::TThinMIn(const TThinMIn &min) : TSBase(), TSIn()
{
    Bf = min.Bf;
    BfL = min.BfL;
    BfC = min.BfC;
...
TMem TMem::GetFromHex(const TStr &Str)
{
    ...
    int ChN = 0;
    while (ChN < StrLen) {
        char MshCh = Str.GetCh(ChN);
        ChN++;
        char LshCh = Str.GetCh(ChN);
        ChN++;
...
/////////////////////////////////////////////////
// Output-Memory
TRefMemOut::TRefMemOut(TMem &_Mem) : TSBase(), TSOut(), Mem(_Mem) {}
...
void TChA::Resize(const int &_MxBfL)
{
    ...
    // which indicates overflow, we resort to max size
        ...
        MxBfL *= 2;
        if (MxBfL < 0 || MxBfL > Max)
            MxBfL = Max;
    }
    // Reserve new buffer
    char *NewBf = new char[MxBfL + 1];
    IAssert(NewBf != NULL);
    // Copy old contents ...
    strcpy(NewBf, Bf);
...
{
    int BChN = 0;
    while ((BChN < BfL) && (GetCh(BChN) <= ' ')) {
        BChN++;
    }
    int EChN = BfL - 1;
    while ((0 <= EChN) && (GetCh(EChN) <= ' ')) {
        EChN--;
    }
    if (BChN <= EChN) {
...
TChA &TChA::ToTrunc()
{
    ...
    int BChN = 0;
    int EChN = StrLen - 1;
    while ((BChN < StrLen) && TCh::IsWs(GetCh(BChN))) {
        BChN++;
    }
    while ((EChN >= 0) && TCh::IsWs(GetCh(EChN))) {
        EChN--;
    }
    if ((BChN != 0) || (EChN != StrLen - 1)) {
        int DstChN = 0;
        for (int SrcChN = BChN; SrcChN <= EChN; SrcChN++) {
            PutCh(DstChN, GetCh(SrcChN));
            DstChN++;
        }
        Trunc(DstChN);
    }
...
void TChA::CompressWs()
{
    ...
    int SrcChN = 0;
    int DstChN = 0;
    while ((SrcChN < StrLen) && TCh::IsWs(GetCh(SrcChN))) {
        SrcChN++;
    }
    while (SrcChN < StrLen) {
    ...
    if ((DstChN > 0) && (TCh::IsWs(GetCh(DstChN - 1)))) {
        DstChN--;
    }
    Trunc(DstChN);
}
...
{
    AssertR(Inner == nullptr || Inner[0] != 0,
            "TStr::Empty string is not nullptr. Fix immediately!");
...
TStr TStr::GetTrunc() const
{
    ...
    while ((BChN < ThisLen) && TCh::IsWs(GetCh(BChN))) {
        BChN++;
    }
    while ((EChN >= 0) && TCh::IsWs(GetCh(EChN))) {
        EChN--;
...
TStr TStr::GetFromHex() const
{
    ...
    int ChN = 0;
    while (ChN < StrLen) {
        char MshCh = GetCh(ChN);
        ChN++;
        char LshCh = GetCh(ChN);
        ChN++;
        uchar Ch = uchar(TCh::GetHex(MshCh) * 16 + TCh::GetHex(LshCh));
...
TStr TStr::LeftOf(const char &SplitCh) const
{
    ...
    int ChN = 0;
    while ((ChN < ThisLen) && (ThisBf[ChN] != SplitCh)) {
        ChN++;
    }
...
TStr TStr::LeftOfLast(const char &SplitCh) const
{
    const char *ThisBf = CStr();
    int ChN = Len() - 1;
    while ((ChN >= 0) && (ThisBf[ChN] != SplitCh)) {
        ChN--;
    }
...
void TStr::SplitLeftOfRightOf(TStr &LStr, const int &LeftOfChN, const int &RightOfChN,
                              TStr &RStr) const
{
    const int StrLen = Len();

    EAssertR(LeftOfChN >= 0 && LeftOfChN <= RightOfChN && RightOfChN < StrLen,
             "Splitting index should be greater than 0 and less than length!");
...
void TStr::SplitOnAllCh(const char &SplitCh, TStrV &StrV, const bool &SkipEmpty) const
{
    ...
    {
        char *BfC = CurStrBf;
        while ((*BfC != 0) && (*BfC != SplitCh)) {
            BfC++;
        }
        bool IsEnd = (*BfC == '\0');
        *BfC = 0;
        if ((BfC > CurStrBf) || (!SkipEmpty)) {
            StrV.Add(TStr(CurStrBf));
        }
        if (IsEnd) {
            break;
        }
...
void TStr::SplitOnAllAnyCh(const TStr &SplitChStr, TStrV &StrV, const bool &SkipEmpty) const
{
    ...
    forever
    {
        ...
        while (*BfC != 0) {
            ...
            while ((*SplitChBfC != 0) && (*SplitChBfC != *BfC)) {
                SplitChBfC++;
            }
            if (*SplitChBfC != 0) {
                break;
...
void TStr::SplitOnWs(TStrV &StrV) const
{
    ...
    forever
    {
        while ((*StrBf != 0) && (TCh::IsWs(*StrBf))) {
            StrBf++;
        }
        char *BfC = StrBf;
        while ((*BfC != 0) && (!TCh::IsWs(*BfC))) {
            BfC++;
        }
        bool IsEnd = (*BfC == '\0');
        *BfC = 0;
...
void TStr::SplitOnNonAlNum(TStrV &StrV) const
{
    ...
    forever
    {
        while ((*StrBf != 0) && (!TCh::IsAlNum(*StrBf))) {
            StrBf++;
        }
        char *BfC = StrBf;
        while ((*BfC != 0) && (TCh::IsAlNum(*BfC))) {
            BfC++;
        }
        bool IsEnd = (*BfC == '\0');
        *BfC = 0;
...
{
    AssertR(SrcCh != '\0' && DstCh != '\0',
            "TStr::ChangeCh: source and destination character should not be the terminator char!");
...
bool TStr::IsHexInt(const bool &Check, const int &MnVal, const int &MxVal, int &Val) const
{
    ...
    if (!Ch.Eof() && !TCh::IsHex(Ch())) {
        return false;
    }
    if (!Ch.Eof())
        _Val = TCh::GetHex(Ch());
    while (TCh::IsHex(Ch.GetCh())) {
        _Val = 16 * _Val + TCh::GetHex(Ch());
    }
    if (Minus) {
        _Val = -_Val;
    }
...
bool TStr::IsHexInt64(const bool &Check, const int64 &MnVal, const int64 &MxVal, int64 &Val) const
{
    // parsing format {ws} [+/-][0x] +{XXX}
    int64 _Val = 0;
    bool Minus = false;
...
bool TStr::IsFlt(const bool &Check, const double &MnVal, const double &MxVal, double &Val,
                 const char &DecDelimCh) const
{
...
bool TStr::IsWord(const bool &WsPrefixP, const bool &FirstUcAllowedP) const
{
    ...
    else if (!FirstUcAllowedP && (TCh::IsUc(Ch()))) {
        return false;
    }
...
bool TStr::IsWcMatch(const int &StrBChN, const TStr &WcStr, const int &WcStrBChN, TStrV &StarStrV,
...
bool TStr::IsWcMatch(const TStr &WcStr, TStrV &StarStrV, const char &StarCh,
                     const char &QuestCh) const
{
...
bool TStr::IsWcMatch(const TStr &WcStr, const char &StarCh, const char &QuestCh) const
{
    TStrV StarStrV;
...
TStr TStr::GetNrFPath(const TStr &FPath)
{
    ...
    if ((NrFPath.Len() >= 2) && isalpha(NrFPath[0]) && (NrFPath[1] == ':')) {
        ...
        if ((NrFPath[2] != '.') && (NrFPath[2] != '/')) {
            NrFPath.Ins(2, "./");
        }
        if (NrFPath[NrFPath.Len() - 1] != '/') {
            NrFPath += "/";
        }
    }
    else {
        if ((NrFPath[0] != '.') && (NrFPath[0] != '/')) {
            NrFPath.Ins(0, "./");
        }
        if (NrFPath[NrFPath.Len() - 1] != '/') {
            NrFPath += "/";
...
bool TStr::IsUInt64(TChRet &Ch, const bool &Check, const uint64 &MnVal, const uint64 &MxVal,
                    uint64 &Val)
{
    EAssert(!Check || (Check && MnVal <= MxVal));
...
TStrIn::TStrIn(const TStr &_Str, const bool &MakeCopyP)
    : TSBase(), TSIn(), OwnP(MakeCopyP), Bf(MakeCopyP ? _Str.CloneCStr() : _Str.CStr()), BfC(0),
      BfL(_Str.Len())
{
}
...
void TStrPool::Resize(const uint &_MxBfL)
{
    ...
    while (newSize < _MxBfL) {
        if (newSize >= GrowBy && GrowBy > 0)
            newSize += GrowBy;
        else if (newSize > 0)
            newSize *= 2;
        else
            newSize = TInt::GetMn(GrowBy, 1024);
        // check for overflow at 4GB
        IAssertR(newSize >= MxBfL, TStr::Fmt("TStrPool::Resize: %u, %u [Size larger than 4Gb, "
                                             "which is not supported by TStrPool]",
                                             newSize, MxBfL)
                                       .CStr());
    }
    if (newSize > MxBfL) {
...
{
    IAssertR(Len > 0,
             "String too short (length includes the null character)"); // J: if (! Len) return -1;
    if (Len == 1 && BfL > 0) {
        return 0;
    } // empty string
    Assert(Str);
...
void TStrPool64::Resize(const ::TSize &_MxBfL)
{
    ...
    while (newSize < _MxBfL) {
        if (newSize >= GrowBy && GrowBy > 0)
            ...
            newSize = (GrowBy > ::TSize(1024)) ? ::TSize(1024) : GrowBy;
    ...
    : MxBfL(StrPool.MxBfL), BfL(StrPool.BfL), GrowBy(StrPool.GrowBy)
{
    ...
    Bf = (char *)malloc(StrPool.MxBfL);
    IAssert(Bf != NULL);
    memcpy(Bf, StrPool.Bf, BfL);
}

TStrPool64::TStrPool64(TSIn &SIn, bool LoadCompact) : MxBfL(0), BfL(0), GrowBy(0), Bf(0)
{
...
char *TInt::SaveFrugalInt(char *pDest, int i)
{
    ...
    // <1xxx xxxx> <1xxx xxxx> <xxxx xxxx> <0xxx xxxx> has 2^29 = 536870912 combinations and is used
    // to store 8319..536879230. <1xxx xxxx> <1xxx xxxx> <xxxx xxxx> <1xxx xxxx> has 2^29 =
    // 536870912 combinations and is used to store -8194..-536879105.
    i++;
    if (i >= 0 && i <= 127) {
        *pDest++ = char(i);
        return pDest;
    ...
    if (i <= -1 && i > -1 - 8192) {
        i = -1 - i;
        *pDest++ = char(0x80 | (i & 0x7f));
        *pDest++ = char(0x40 | ((i >> 7) & 0x3f));
        return pDest;
    }
    if (i >= 128 + 8192 && i < 128 + 8192 + 536870912) {
        i -= 128 + 8192;
        *pDest++ = char(0x80 | (i & 0x7f));
        *pDest++ = char(0x80 | ((i >> 7) & 0x7f));
        *pDest++ = char((i >> 14) & 0xff);
        *pDest++ = char((i >> 22) & 0x7f);
        return pDest;
    }
    if (i <= -1 - 8192 && i > -1 - 8192 - 536870912) {
        i = (-1 - 8192) - i;
        *pDest++ = char(0x80 | (i & 0x7f));
        *pDest++ = char(0x80 | ((i >> 7) & 0x7f));
        *pDest++ = char((i >> 14) & 0xff);
        *pDest++ = char(0x80 | ((i >> 22) & 0x7f));
        return pDest;
    }
    IAssertR(false, TInt::GetStr(i));
    return 0;
}
...
{
    TStrV IpStrV;
    IpStr.SplitOnAllCh(SplitCh, IpStrV);
    Ip = 0;
    int Byte = 0;
    if (IpStrV.Len() != 4) {
        return false;
    }
    if (!IpStrV[0].IsInt(true, 0, 255, Byte)) {
        return false;
    };
    Ip = (uint)Byte;
    if (!IpStrV[1].IsInt(true, 0, 255, Byte)) {
        return false;
    };
    Ip = (Ip << 8) | (uint)Byte;
    if (!IpStrV[2].IsInt(true, 0, 255, Byte)) {
        return false;
    };
    Ip = (Ip << 8) | (uint)Byte;
    if (!IpStrV[3].IsInt(true, 0, 255, Byte)) {
        return false;
    };
    Ip = (Ip << 8) | (uint)Byte;
    return true;
...
uint TUInt::GetUIntFromIpStr(const TStr &IpStr, const char &SplitCh)
{
    ...
    uint Ip = 0;
    int Byte = 0;
    EAssertR(IpStrV[0].IsInt(true, 0, 255, Byte), TStr::Fmt("Bad IP: '%s;", IpStr.CStr()));
    Ip = (uint)Byte;
    EAssertR(IpStrV[1].IsInt(true, 0, 255, Byte), TStr::Fmt("Bad IP: '%s;", IpStr.CStr()));
    Ip = (Ip << 8) | (uint)Byte;
    EAssertR(IpStrV[2].IsInt(true, 0, 255, Byte), TStr::Fmt("Bad IP: '%s;", IpStr.CStr()));
    Ip = (Ip << 8) | (uint)Byte;
    EAssertR(IpStrV[3].IsInt(true, 0, 255, Byte), TStr::Fmt("Bad IP: '%s;", IpStr.CStr()));
    Ip = (Ip << 8) | (uint)Byte;
    return Ip;
}
...
{
    return TStr::Fmt("%d.%d.%d.%d", ((Ip >> 24) & 0xFF), ((Ip >> 16) & 0xFF), ((Ip >> 8) & 0xFF),
                     (Ip & 0xFF));
}
...
{
    TStrV IpStrV;
    IpStr.SplitOnAllCh(SplitCh, IpStrV, false);
    ...
    for (int IpStrN = 0; IpStrN < IpStrV.Len(); IpStrN++) {
        if (IpStrV[IpStrN].Empty()) {
            continue;
        }
        if (IpStrV[IpStrN].IsHexInt(true, 0x0000, 0xFFFF, Group)) {
            continue;
        }
        return false;
    }
...
{
    XSaveBETagArg4(Nm, "MnX", TFlt::GetStr(double(MnX)), "MnY", TFlt::GetStr(double(MnY)), "MxX",
                   TFlt::GetStr(double(MxX)), "MxY", TFlt::GetStr(double(MxY)));
}
...
TStr TFltRect::GetStr() const
{
    TChA ChA;
    ChA += '(';
    ChA += TFlt::GetStr(MnX, "%0.2f");
    ChA += ',';
    ChA += TFlt::GetStr(MnY, "%0.2f");
    ChA += ',';
    ChA += TFlt::GetStr(MxX, "%0.2f");
    ChA += ',';
    ChA += TFlt::GetStr(MxY, "%0.2f");
    ChA += ')';
    return ChA;
}
