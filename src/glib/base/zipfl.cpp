...
TZipIn::TZipIn(const TStr& FNm)
    : TSBase(), TSIn(), ZipStdoutRd(NULL), ZipStdoutWr(NULL), SNm(FNm.CStr()), FLen(0), CurFPos(0),
      Bf(NULL), BfC(0), BfL(0)
{
    ...
    //  FNm.GetFExt().CStr()); EFailR(TStr::Fmt("File %s: compression format %s not supported",
    //  FNm.CStr(), FNm.GetFExt().CStr()).CStr());
    //}
    ...
    EAssertR(CreatePipe(&ZipStdoutRd, &ZipStdoutWr, &saAttr, PipeBufferSz),
...
TZipIn::TZipIn(const TStr& FNm, bool& OpenedP)
    : TSBase(), TSIn(), ZipStdoutRd(NULL), ZipStdoutWr(NULL), SNm(FNm.CStr()), FLen(0), CurFPos(0),
      Bf(NULL), BfC(0), BfL(0)
{
    EAssertR(!FNm.Empty(), "Empty file-name.");
...
uint64 TZipIn::GetFLen(const TStr& ZipFNm)
{
    ...
    // CreateZipProcess(GetCmd(FNm), FNm);
    {
        ...
        siStartInfo.dwFlags |= STARTF_USESTDHANDLES;
        // Create the child process.
        const BOOL FuncRetn = CreateProcess(NULL, (LPSTR)CmdLine.CStr(), NULL, NULL, TRUE, 0, NULL,
...
#ifdef GLib_WIN
    DWORD BytesOut;
    EAssertR(WriteFile(ZipStdinWr, Bf, DWORD(BfL), &BytesOut, NULL) != 0,
...
TZipOut::TZipOut(const TStr& FNm)
    : TSBase(), TSOut(), ZipStdinRd(NULL), ZipStdinWr(NULL), SNm(FNm.CStr()), Bf(NULL), BfL(0)
{
    EAssertR(!FNm.Empty(), "Empty file-name.");
#ifdef GLib_WIN
    // create pipes
