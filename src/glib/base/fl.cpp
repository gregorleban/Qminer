...
int TSOut::UpdateLnLen(const int& StrLen, const bool& ForceInLn)
{
    int Cs = 0;
    if (MxLnLen != -1) {
        if ((!ForceInLn) && (LnLen + StrLen > MxLnLen)) {
            Cs += PutLn();
        }
        LnLen += StrLen;
    }
...
int TSOut::PutDosLn(const int& Lns)
{
    ...
    int Cs = 0;
    for (int LnN = 0; LnN < Lns; LnN++) {
        Cs += PutCh(TCh::CrCh) + PutCh(TCh::LfCh);
    }
    return Cs;
}
...
void TFIn::FillBf()
{
    EAssertR((BfC == BfL) && ((BfL == -1) || (BfL == MxBfL)),
...
TMIn::TMIn(TSIn& SIn) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0)
{
    BfL = SIn.Len();
    Bf = new char[BfL];
    for (int BfC = 0; BfC < BfL; BfC++) {
...
TMIn::TMIn(const char* CStr) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0)
{
    BfL = int(strlen(CStr));
    Bf = new char[BfL + 1];
    strcpy(Bf, CStr);
...
TMIn::TMIn(const TStr& Str) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0)
{
    BfL = Str.Len();
    Bf = new char[BfL];
    strncpy(Bf, Str.CStr(), BfL);
}

TMIn::TMIn(const TChA& ChA) : TSBase(), TSIn(), Bf(NULL), BfC(0), BfL(0)
{
    BfL = ChA.Len();
    Bf = new char[BfL];
    strncpy(Bf, ChA.CStr(), BfL);
}
...
void TMOut::Resize(const int& ReqLen)
{
    ...
    if (Bf == NULL) {
        IAssert(MxBfL == 0);
        if (ReqLen < 0)
            Bf = new char[MxBfL = 1024];
        else
            Bf = new char[MxBfL = ReqLen];
    }
    else {
        if (ReqLen < 0) {
            MxBfL *= 2;
        }
        else if (ReqLen < MxBfL) {
            return;
        } // nothing to do
        else {
            MxBfL = (2 * MxBfL < ReqLen ? ReqLen : 2 * MxBfL);
        }
        char* NewBf = new char[MxBfL];
        memmove(NewBf, Bf, BfL);
        delete[] Bf;
        Bf = NewBf;
    }
}

TMOut::TMOut(const int& _MxBfL) : TSBase(), TSOut(), Bf(NULL), BfL(0), MxBfL(0), OwnBf(true)
{
...
TMOut::TMOut(char* _Bf, const int& _MxBfL)
    : TSBase(), TSOut(), Bf(_Bf), BfL(0), MxBfL(_MxBfL), OwnBf(false)
{
}
...
void TMOut::CutBf(const int& CutBfL)
{
    ...
    if (CutBfL == BfL) {
        ...
        memmove(Bf, Bf + CutBfL, BfL - CutBfL);
        BfL = BfL - CutBfL;
    }
}
...
{
    for (int BfC = 0; BfC < BfL; BfC++) {
        if ((Bf[BfC] == TCh::CrCh) && ((BfC + 1 < BfL) && (Bf[BfC + 1] == TCh::LfCh))) {
            return true;
        }
    }
    return false;
...
{
    for (int BfC = 0; BfC < BfL; BfC++) {
        if ((Bf[BfC] == TCh::CrCh) || (Bf[BfC] == TCh::LfCh)) {
            return true;
        }
    }
    return false;
...
bool TFile::Move(const TStr& SrcFNm, const TStr& DstFNm, const bool& ThrowExceptP,
                 const bool& FailIfExistsP)
{
    return MoveFileEx(SrcFNm.CStr(), DstFNm.CStr(),
                      FailIfExistsP ? MOVEFILE_COPY_ALLOWED
                 ...
                 const bool& FailIfExistsP)
{
    ...
    if ((input = open(SrcFNm.CStr(), O_RDONLY)) == -1) {
        if (ThrowExceptP) {
            TExcept::Throw(
                TStr::Fmt("Error copying file '%s' to '%s': cannot open source file for reading.",
                          SrcFNm.CStr(), DstFNm.CStr()));
        }
        else {
            return;
    ...
    if ((source = mmap(0, filesize, PROT_READ, MAP_SHARED, input, 0)) == (void*)-1) {
        ...
        close(output);
        if (ThrowExceptP) {
            TExcept::Throw(TStr::Fmt("Error copying file '%s' to '%s': cannot mmap input file.",
                                     SrcFNm.CStr(), DstFNm.CStr()));
        }
        else {
            return;
    ...
    if ((target = mmap(0, filesize, PROT_WRITE, MAP_SHARED, output, 0)) == (void*)-1) {
        ...
        close(output);
        if (ThrowExceptP) {
            TExcept::Throw(TStr::Fmt("Error copying file '%s' to '%s': cannot mmap output file.",
                                     SrcFNm.CStr(), DstFNm.CStr()));
        }
        else {
            return;
...
{
    EAssertR(rename(SrcFNm.CStr(), DstFNm.CStr()) == 0,
...
TStr TFile::GetUniqueFNm(const TStr& FNm)
{
    ...
    {
        NewFNm = TmpFNm;
        NewFNm.ChangeStr("#", TStr::Fmt("%03d", Cnt));
        Cnt++;
        if (!TFile::Exists(NewFNm)) {
            break;
        }
    }
    return NewFNm;
...
{
    // open
    HANDLE hFile = CreateFile(FNm.CStr(),                         // file to open
...
uint64 TFile::GetCreateTm(const TStr& FNm)
{
    ...
    CloseHandle(hFile);
    // convert to uint64
    TUInt64 UInt64(uint(lpCreationTime.dwHighDateTime), uint(lpCreationTime.dwLowDateTime));
...
uint64 TFile::GetLastAccessTm(const TStr& FNm)
{
    ...
    CloseHandle(hFile);
    // convert to uint64
    TUInt64 UInt64(uint(lpLastAccessTime.dwHighDateTime), uint(lpLastAccessTime.dwLowDateTime));
    return UInt64.Val / uint64(10000);
}
...
uint64 TFile::GetLastWriteTm(const TStr& FNm)
{
    ...
    CloseHandle(hFile);
    // convert to uint64
    TUInt64 UInt64(uint(lpLastWriteTime.dwHighDateTime), uint(lpLastWriteTime.dwLowDateTime));
    return UInt64.Val / uint64(10000);
}
