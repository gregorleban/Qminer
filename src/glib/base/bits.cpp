...
TB1Def::TB1Def()
{
    B1P2T = new TB1[B1Bits + 1];
    B1P2T[0] = 1;
    for (int BitN = 1; BitN < B1Bits; BitN++) {
        B1P2T[BitN] = TB1(2 * B1P2T[BitN - 1]);
    }
    B1P2T[B1Bits] = 0;
    ...
    for (int B1N = 0; B1N < MxB1 + 1; B1N++) {
        TB1 B1 = (TB1)B1N;
        B1BitsT[B1] = 0;
        for (int BitN = 0; BitN < B1Bits; BitN++) {
            B1BitsT[B1N] += B1 % 2;
            B1 /= (TB1)2;
        }
    }
...
{
    int L2 = 0;
    while ((L2 < TB1Def::MxP2Exp) && (Val >= B1Def.B1P2T[L2])) {
        L2++;
    }
    return L2 - 1;
}
...
TB2Def::TB2Def()
{
    B2P2T = new TB2[B2Bits + 1];
    B2P2T[0] = 1;
    for (int BitN = 1; BitN < B2Bits; BitN++) {
        B2P2T[BitN] = TB2(2 * B2P2T[BitN - 1]);
    }
    B2P2T[B2Bits] = 0;
}
...
{
    return TB1Def::B1Def.B1BitsT[(B2 >> (0 * TB1Def::B1Def.B1Bits)) & TB1Def::B1Def.MxB1] +
...
{
    int L2 = 0;
    while ((L2 < TB2Def::MxP2Exp) && (Val >= B2Def.B2P2T[L2])) {
        L2++;
    }
    return L2 - 1;
}
...
TB4Def::TB4Def()
{
    B4P2T = new TB4[B4Bits + 1];
    B4P2T[0] = 1;
    for (int BitN = 1; BitN < B4Bits; BitN++) {
        B4P2T[BitN] = TB4(2 * B4P2T[BitN - 1]);
    }
    B4P2T[B4Bits] = 0;
}
...
{
    return TB1Def::B1Def.B1BitsT[(B4 >> (0 * TB1Def::B1Def.B1Bits)) & TB1Def::B1Def.MxB1] +
...
{
    int L2 = 0;
    while ((L2 < TB4Def::MxP2Exp) && (Val >= B4Def.B4P2T[L2])) {
        L2++;
    }
    return L2 - 1;
}
...
TFSet::TFSet(const int &FlagN1, const int &FlagN2, const int &FlagN3, const int &FlagN4,
             const int &FlagN5, const int &FlagN6, const int &FlagN7, const int &FlagN8,
             const int &FlagN9)
    : B4V(4, 4)
{
    if (FlagN1 != -1) {
...
/////////////////////////////////////////////////
// Bit-Set
TBSet::TBSet(const TBSet &BSet) : B4s(0), Bits(0), LastB4Mask(0), B4T(NULL)
{
    Gen(BSet.Bits);
...
bool TBSet::operator==(const TBSet &BSet) const
{
    ...
    for (int B4N = 0; B4N < B4s - 1; B4N++) {
        if (B4T[B4N] != BSet.B4T[B4N]) {
            return false;
        }
    }
...
int TBSet::Get1s()
{
    ...
    SetLastB4();
    for (int B4N = 0; B4N < B4s; B4N++) {
        Ones += TB4Def::B4Def.GetB4Bits(B4T[B4N]);
    }
    return Ones;
}
