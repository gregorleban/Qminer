...
// Cache
/*
Used to cache data until we are storing MxMemUsed bytes of data. At that time we remove
*/
template <class TKey, class TDat, class THashFunc = TDefaultHashFunc<TKey> >
class TCache {
public:
...
private:
    ...
    TKeyL TimeKeyL;
    void* RefToBs;
    /// purge data that wasn't used for the most time
...
void TCache<TKey, TDat, THashFunc>::ChangeKey(const TKey& OldKey, const TKey& NewKey) {
    ...
    const int OldKeyId = KeyDatH.GetKeyId(OldKey);
...
void TCache<TKey, TDat, THashFunc>::Flush() {
    ...
    while (KeyDatH.FNextKeyId(KeyId)) {
        if (Done % 10000 == 0) {
            const double Perc = Done / (0.01 * (double) KeyDatH.Len());
