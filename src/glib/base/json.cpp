...
TJsonVal::TJsonVal(TSIn &SIn)
    : JsonValType((TJsonValType)(TInt(SIn).Val)), Bool(SIn), Num(SIn), Str(SIn), ValV(SIn),
      KeyValH(SIn)
{
}
...
{
    return JsonValType == JsonVal.JsonValType && Bool == JsonVal.Bool && Num == JsonVal.Num &&
           Str == JsonVal.Str && ValV == JsonVal.ValV && KeyValH == JsonVal.KeyValH;
}
...
void TJsonVal::AssertObjKey(const TStr &Key, const TStr &Fun)
{
    ...
    if (!IsObjKey(Key)) {
        throw TExcept::New("Exception in function `" + Fun + "`: missing JSON property:`" + Key +
                           "`.");
    }
}
...
void TJsonVal::AssertObjKeyStr(const TStr &Key, const TStr &Fun)
{
    ...
    if (!GetObjKey(Key)->IsStr()) {
        throw TExcept::New("Exception in function `" + Fun + "`: JSON property:`" + Key +
                           "` is not a string.");
    }
}
...
uint64 TJsonVal::GetMemUsed() const
{
    return sizeof(TJsonVal) +
           ...
           TMemUtils::GetExtraMemberSize(Bool) + TMemUtils::GetExtraMemberSize(Num) +
           TMemUtils::GetExtraMemberSize(Str) + TMemUtils::GetExtraMemberSize(ValV) +
           TMemUtils::GetExtraMemberSize(KeyValH);
}

PJsonVal TJsonVal::GetValFromLx(TILx &Lx)
{
    static TFSet ValExpect =
        TFSet() | syIdStr | syFlt | syQStr | syLBracket | syLBrace | syRBracket;
    ...
    if ((Lx.Sym == syIdStr) && (Lx.Str == "null")) {
        ...
        Val->PutArr();
        Lx.GetSym(ValExpect); // added ValExpect to correctly parse arrays of floats
        if (Lx.Sym != syRBracket) {
            forever
            {
                ...
                else if (Lx.Sym == syRBracket) {
                    break;
                }
                else {
                    TExcept::Throw("JSON Array not properly formed.");
                }
            }
        }
        Lx.GetSym();
        ...
        Val->PutObj();
        Lx.GetSym(TFSet() | syRBrace | syQStr);
        if (Lx.Sym != syRBrace) {
            forever
            {
                TStr SubKey = Lx.Str;
                ...
                if (Lx.Sym == syComma) {
                    Lx.GetSym(TFSet() | syQStr);
                }
                else if (Lx.Sym == syRBrace) {
                    break;
                }
                else {
                    TExcept::Throw("JSON Object not properly formed.");
                }
            }
        }
        Lx.GetSym();
...
void TJsonVal::GetChAFromVal(const PJsonVal &Val, TChA &ChA)
{
    ...
    case jvtObj:
        ...
        for (int ObjKeyN = Val->KeyValH.FFirstKeyId(); Val->KeyValH.FNextKeyId(ObjKeyN);) {
            ...
            TStr ObjKey;
            PJsonVal ObjVal;
            Val->GetObjKeyVal(ObjKeyN, ObjKey, ObjVal);
        ...
        break;
    default:
        TExcept::Throw("Error serializing json to string");
    }
}
...
void TBsonObj::CreateBsonRecursive(const TJsonVal &JsonVal, TStrHash<TInt, TBigStrPool> *Voc,
                                   TSOut &SOut)
{
    TJsonValType t = JsonVal.GetJsonValType();
    if (t == jvtUndef) {
        ...
        SOut.Save((char)2);
        SOut.Save(JsonVal.GetBool());
    }
    else if (t == jvtNum) {
        SOut.Save((char)3);
        SOut.Save(JsonVal.GetNum());
    }
    else if (t == jvtStr) {
        SOut.Save((char)4);
