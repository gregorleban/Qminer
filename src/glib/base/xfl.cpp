...
TFFile::TFFile(const TStrV& _FPathV, const TStrV& _FExtV, const TStr& _FBaseWc,
    ...
    : FPathV(_FPathV), FExtV(_FExtV), FBaseWc(_FBaseWc), CsImpP(false), RecurseP(_RecurseP),
      FPathN(0 - 1), FFileDesc(TFFileDesc::New()), SubFFile(), CurFNm(), CurFNmN(0 - 1)
{
    // prepare file-paths
...
bool TFFile::Next(TStr& FNm)
{
    ...
    while (FPathN < FPathV.Len()) {
        if ((FPathN != -1) && (FindNextFile(FFileDesc->FFileH, &FFileDesc->FDesc))) {
            ...
            while ((++FPathN < FPathV.Len()) &&
                   ((FFileDesc->FFileH =
                         FindFirstFile((FPathV[FPathN] + "*.*").CStr(), &FFileDesc->FDesc)) ==
        ...
        while (true) {
            ...
            if (!FFileDesc->FDesc) {
                if ((++FPathN) < FPathV.Len()) {
                    ...
                    break;
                if (!FFileDesc->FDesc)
                    break; // failed to open this one; pass control to outer loop
            }
            ...
            if (FFileDesc->DirEnt) {
                ...
                // end of directory; clean up (ignore DirEnt, it's allocated within FDesc), pass
                // control to outer loop
...
void TFFile::GetFNmV(const TStr& FPath, const TStrV& FExtV, const bool& RecurseP, TStrV& FNmV)
{
    // prepare file-directory traversal
    TStrV FPathV;
    FPathV.Add(FPath);
    TFFile FFile(FPathV, FExtV, "", RecurseP);
    TStr FNm;
    // traverse directory
    FNmV.Clr();
...
void TFFile::GetFNmV(const TStr& FPath, const TStr& FExt, const bool& RecurseP, TStrV& FNmV)
{
...
{
    TStrV PartsV;
    TDir::SplitPath(FileWithDir, PartsV);
    if (PartsV.Len() > 0)
...
void TFPathNotify::UpdateSOut(const TTm& Tm)
{
    ...
    if (Rollover == fpnrDay) {
        ...
        FNm += TStr::Fmt("-Y%04d-M%02d-D%02d-H%02d", Tm.GetYear(), Tm.GetMonth(), Tm.GetDay(),
                         Tm.GetHour());
    }
    FNm += ".log";
...
TFPathNotify::TFPathNotify(const TStr& _LogFPath, const TStr& _PrefixFNm, const bool& _FlushP,
                           const TFPathNotifyRollover& _Rollover)
    : LogFPath(_LogFPath), PrefixFNm(_PrefixFNm), Rollover(_Rollover), FlushP(_FlushP)
{
...
//////////////////////////////////////
// File-Notify
TFileNotify::TFileNotify(const TStr& _FileName, const bool& _AddTimeStamp,
                         const bool& _SeparateFilesForEachDay, const bool& _FlushEachWrite)
    : FileName(_FileName), AddTimeStamp(_AddTimeStamp),
      SeparateFilesForEachDay(_SeparateFilesForEachDay), FlushEachWrite(_FlushEachWrite)
{
...
void TFileNotify::OpenNewFileForDate()
{
    ...
    TStr Path, FName;
    FileName.SplitOnLastCh(Path, '/', FName);
    TStr FullNm = Path;
    if (Path != "")
        FullNm += "/";
...
void TFileNotify::OnStatus(const TStr& MsgStr)
{
    ...
    if (AddTimeStamp) {
        ...
        if (SeparateFilesForEachDay) {
            ...
            File->PutStrFmt("[%s %s] ", NowTm.GetYMDDashStr().CStr(),
    ...
    if (AddTimeStamp) {
        ...
        if (SeparateFilesForEachDay)
            ...
            File->PutStrFmt("[%s %s] %s: %s\n", NowTm.GetYMDDashStr().CStr(),
                            NowTm.GetHMSTColonDotStr(true, false).CStr(), TypeStr.CStr(),
                            MsgStr.CStr());
    }
    else
        File->PutStrFmt("%s: %s\n", TypeStr.CStr(), MsgStr.CStr());
...
/////////////////////////////////////////////////
// File-Lock
TFileLock::TFileLock(const TStr& _LockFNm) : LockFNm(_LockFNm), LockId(TGuid::GenGuid()) {}
