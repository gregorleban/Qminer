...
void THtmlLxChDef::SetUcCh(const char& UcCh, const char& LcCh)
{
    ...
    IAssert((UcChV[LcCh - TCh::Mn] == TCh(0)) || (UcChV[LcCh - TCh::Mn] == TCh(LcCh)));
...
TStr THtmlLxChDef::GetEscStr(const TStr& Str) const
{
    ...
    else if ((Str.Len() >= 2) && (Str[0] == '&') && (Str[1] == '#')) {
        int ChCd = 0;
        for (int ChN = 2; ChN < Str.Len(); ChN++) {
            if (ChCd <= 0xFFFF) {
                ChCd = ChCd * 10 + Str[ChN] - '0';
            }
        }
...
THtmlLxChDef::THtmlLxChDef() : ChTyV(TCh::Vals), UcChV(TCh::Vals), LcChV(TCh::Vals), EscStrH(100)
{
    ...
    for (int Ch = TCh::Mn; Ch <= TCh::Mx; Ch++) {
        if ((Ch < 0) || (127 < Ch)) {
            SetChTy(hlctAlpha, TStr(TCh(char(Ch))));
        }
    }
    ...
    SetEscStr("&aring", "\xe5");
    SetEscStr("&Aring", "\xc5");
    SetEscStr("&oslash", "\xf8");
    SetEscStr("&Oslash", "\xd8");
    SetEscStr("&Aelig", "\xc6");
    SetEscStr("&aelig", "\xe6");
...
void THtmlLx::GetTag()
{
    ...
    if (DoParseArg) {
        while ((Ch != '>') && (Ch != TCh::EofCh)) {
            while ((!ChDef.IsAlpha(Ch)) && (Ch != '>') && (Ch != TCh::EofCh)) {
                GetCh();
            }
            if (ChDef.IsAlpha(Ch)) {
                ArgNm.Clr();
...
{
    if (DoUc) {
        return PHtmlTok(new THtmlTok(Sym, UcChA, ArgNmValV));
    }
    else {
        return PHtmlTok(new THtmlTok(Sym, ChA, ArgNmValV));
    }
}
...
TStr THtmlLx::GetFullBTagStr() const
{
    ...
    for (int ArgN = 0; ArgN < GetArgs(); ArgN++) {
        BTagChA += ' ';
        BTagChA += GetArgNm(ArgN);
        BTagChA += '=';
        BTagChA += '"';
        BTagChA += GetArgVal(ArgN);
        BTagChA += '"';
    }
    BTagChA += '>';
    return BTagChA;
...
void THtmlLx::MoveToBTag3OrEof(const TStr& TagNm1, const TStr& TagNm2, const TStr& TagNm3)
{
    do {
        GetSym();
             ...
             ((Sym != hsyBTag) || ((UcChA != TagNm1) && (UcChA != TagNm2) && (UcChA != TagNm3))));
}
...
void THtmlLx::MoveToBTagOrETagOrEof(const TStr& BTagNm, const TStr& ETagNm)
{
    do {
        GetSym();
    } while ((Sym != hsyEof) && ((Sym != hsyBTag) || (UcChA != BTagNm)) &&
             ((Sym != hsyETag) || (UcChA != ETagNm)));
}

void THtmlLx::MoveToBTagArgOrEof(const TStr& TagNm, const TStr& ArgNm, const TStr& ArgVal)
{
    forever
    {
        GetSym();
        ...
        if ((Sym == hsyBTag) && (UcChA == TagNm) && (IsArg(ArgNm)) && (GetArg(ArgNm) == ArgVal)) {
            break;
        }
    }
}

void THtmlLx::MoveToBTagArg2OrEof(const TStr& TagNm, const TStr& ArgNm1, const TStr& ArgVal1,
                                  const TStr& ArgNm2, const TStr& ArgVal2, const bool& AndOpP)
{
    forever
    {
        ...
        if (AndOpP) {
            if ((Sym == hsyBTag) && (UcChA == TagNm) && (IsArg(ArgNm1)) &&
                (GetArg(ArgNm1) == ArgVal1) && (IsArg(ArgNm2)) && (GetArg(ArgNm2) == ArgVal2)) {
                break;
            }
        }
        else {
            if ((Sym == hsyBTag) && (UcChA == TagNm) &&
                (((IsArg(ArgNm1)) && (GetArg(ArgNm1) == ArgVal1)) ||
                 ((IsArg(ArgNm2)) && (GetArg(ArgNm2) == ArgVal2)))) {
                break;
            }
        }
    }
...
void THtmlLx::MoveToBTagOrEof(const TStr& TagNm1, const TStr& ArgNm1, const TStr& ArgVal1,
                              const TStr& TagNm2, const TStr& ArgNm2, const TStr& ArgVal2)
{
    forever
    {
        ...
        if ((Sym == hsyBTag) && (UcChA == TagNm1) && (IsArg(ArgNm1)) &&
            (GetArg(ArgNm1) == ArgVal1)) {
            break;
        }
        if ((Sym == hsyBTag) && (UcChA == TagNm2) && (IsArg(ArgNm2)) &&
...
TStr THtmlLx::GetStrToBTag(const TStr& TagNm, const TStr& ArgNm, const TStr& ArgVal,
                           const bool& TxtOnlyP)
{
    TChA OutChA;
    forever
    {
        GetSym();
        if ((Sym == hsyEof) ||
            ((Sym == hsyBTag) && (UcChA == TagNm) && (IsArg(ArgNm)) && (GetArg(ArgNm) == ArgVal))) {
            break;
        }
        else {
            if (PreSpaces > 0) {
...
TStr THtmlLx::GetStrToETag2(const TStr& TagNm1, const TStr& TagNm2, const bool& TxtOnlyP)
{
    TChA OutChA;
    forever
    {
        GetSym();
        if ((Sym == hsyEof) || ((Sym == hsyETag) && (UcChA == TagNm1)) ||
            ((Sym == hsyETag) && (UcChA == TagNm2))) {
            break;
        }
        else {
            if (PreSpaces > 0) {
...
TStr THtmlLx::GetHRefBeforeStr(const TStr& Str)
{
    ...
    forever
    {
        ...
        if ((Sym == hsyBTag) && (UcChA == "<A>")) {
            HRefStr = GetArg("HREF");
        }
        if ((Sym == hsyStr) && (ChA == Str)) {
            break;
        }
    }
    return HRefStr;
...
TStr THtmlTok::GetFullStr() const
{
    if ((Sym == hsyBTag) && (ArgNmValV.Len() > 0)) {
        ...
        for (int ArgNmValN = 0; ArgNmValN < ArgNmValV.Len(); ArgNmValN++) {
            FullChA += ' ';
            FullChA += ArgNmValV[ArgNmValN].Key;
            FullChA += '=';
            FullChA += '"';
            FullChA += ArgNmValV[ArgNmValN].Dat;
            FullChA += '"';
        }
        FullChA += '>';
        return FullChA;
        ...
        FullChA += '<';
        FullChA += '/';
        FullChA += Str.GetSubStr(1, Str.Len() - 1);
        return FullChA;
    }
    else {
        return GetStr();
    ...
    if (GetSym() == hsyBTag) {
        ...
        if ((TagNm == ATagNm) && (IsArg(HRefArgNm))) {
            ...
            if ((HttpEquivArgVal == "REFRESH") && IsArg("CONTENT")) {
                ...
                RelUrlStr = ContentStr.GetSubStr(LeftStr.Len() + UrlEqStr.Len(), ContentStr.Len());
...
{
    if (TxtMode) {
        SOut->PutStr(GetFullStr());
        SOut->PutStr(" ");
    }
    else {
        SOut->PutStr(THtmlLx::GetSymStr(Sym));
        SOut->PutStr(" ");
        SOut->PutStr(GetFullStr());
        SOut->PutStr(" ");
    }
...
bool THtmlTok::IsBreakTag(const TStr& TagNm)
{
    ...
    if (BreakTagNmH.Len() == 0) {
        BreakTagNmH.AddKey(TStr("<H1>"));
        BreakTagNmH.AddKey(TStr("<H2>"));
        BreakTagNmH.AddKey(TStr("<H3>"));
        BreakTagNmH.AddKey(TStr("<H4>"));
        BreakTagNmH.AddKey(TStr("<H5>"));
        BreakTagNmH.AddKey(TStr("<H6>"));
        BreakTagNmH.AddKey(TStr("<BR>"));
        BreakTagNmH.AddKey(TStr("<HR>"));
        BreakTagNmH.AddKey(TStr("<P>"));
        BreakTagNmH.AddKey(TStr("<DL>"));
        BreakTagNmH.AddKey(TStr("<UL>"));
        BreakTagNmH.AddKey(TStr("<OL>"));
        BreakTagNmH.AddKey(TStr("<LI>"));
        BreakTagNmH.AddKey(TStr("<DT>"));
        BreakTagNmH.AddKey(TStr("<DD>"));
        BreakTagNmH.AddKey(TStr("<HEAD>"));
        BreakTagNmH.AddKey(TStr("<TITLE>"));
        BreakTagNmH.AddKey(TStr("<META>"));
        BreakTagNmH.AddKey(TStr("<SCRIPT>"));
        BreakTagNmH.AddKey(TStr("<HEAD>"));
        BreakTagNmH.AddKey(TStr("<BODY>"));
    }
...
/////////////////////////////////////////////////
// Html-Document
THtmlDoc::THtmlDoc(const PSIn& SIn, const THtmlDocType& Type, const bool& DoUc) : TokV(1000, 0)
{
    THtmlLx Lx(SIn);
    bool MkTok = false;
    bool InUL = false;
    while (Lx.GetSym() != hsyEof) {
        ...
        case hdtStr:
            ...
            break;
        case hdtStrNum:
            MkTok = (Lx.Sym == hsyStr) || (Lx.Sym == hsyNum);
            break;
        case hdtTag:
            MkTok = (Lx.Sym == hsyBTag) || (Lx.Sym == hsyETag);
            break;
        case hdtA:
            MkTok = (Lx.Sym == hsyBTag) && (Lx.UcChA == THtmlTok::ATagNm);
            break;
        case hdtHRef:
            ...
            break;
        case hdtUL:
            if ((Lx.Sym == hsyBTag) && (Lx.UcChA == THtmlTok::UlTagNm)) {
                InUL = true;
            }
            MkTok = InUL;
            if ((Lx.Sym == hsyETag) && (Lx.UcChA == THtmlTok::UlTagNm)) {
                InUL = false;
            }
            break;
        default:
...
TStr THtmlDoc::GetTxtLnDoc(const TStr& HtmlStr)
{
    ...
    while (HtmlLx.GetSym() != hsyEof) {
        ...
        case hsySSym:
            ...
            if (HtmlLx.PreSpaces > 0) {
                LnDocChA += ' ';
            }
            LnDocChA += Str.CStr();
            break;
        case hsyBTag:
            if ((!LnDocChA.Empty()) && (LnDocChA.LastCh() != ' ')) {
                LnDocChA += ' ';
            }
            if ((!InScript) && (Str == "<SCRIPT>")) {
                InScript = true;
            }
            break;
        case hsyETag:
...
TStr THtmlDoc::GetTxtLnDoc(const TStr& HtmlStr, const TStr& BaseUrlStr, const bool& OutUrlP,
                           const bool& OutTagsP)
{
...
void THtmlDoc::SaveHtmlToTxt(const TStr& HtmlStr, const PSOut& TxtSOut, const TStr& BaseUrlStr,
...
void THtmlDoc::SaveHtmlToTxt(const TStr& HtmlStr, const TStr& TxtFNm, const TStr& BaseUrlStr,
                             const bool& OutUrlP, const bool& OutTagsP)
{
    // create output file
...
void THtmlDoc::SaveHtmlToXml(const TStr& HtmlStr, const PSOut& XmlSOut, const TStr& BaseUrlStr,
                ...
                if (OutTagsP) {
                    if (OutArgsP) {
                        ...
                        for (int ArgN = 0; ArgN < HtmlLx.GetArgs(); ArgN++) {
                            ...
                            fprintf(fXml, "    <Arg Nm=\"%s\" Val=\"%s\"/>", ArgNm.CStr(),
                                    ArgVal.CStr());
                        }
                        fprintf(fXml, "  </BTag>\n");
                    }
...
void THtmlDoc::SaveHtmlToXml(const TStr& HtmlStr, const TStr& XmlFNm, const TStr& BaseUrlStr,
                             const bool& OutTextP, const bool& OutUrlP, const bool& OutToksP,
                             const bool& OutTagsP, const bool& OutArgsP)
{
    // create output file
    ...
    // save to output file
    SaveHtmlToXml(HtmlStr, XmlSOut, BaseUrlStr, OutTextP, OutUrlP, OutToksP, OutTagsP, OutArgsP);
}
...
bool THtmlDoc::_IsTagRedir(const TStr& TagStr, const TStr& ArgNm, THtmlLx& Lx,
...
TStr THtmlDoc::GetRedirHtmlDocStr(const TStr& HtmlStr, const TStr& BaseUrlStr,
    ...
    for (int TokN = 0; TokN < RefHtmlDoc->GetToks(); TokN++) {
        ...
        if ((TokSym == hsyBTag) && (TokStr == THtmlTok::ATagNm)) {
            ...
            forever
            {
                ...
                if ((ATokSym == hsyETag) && (ATokStr == THtmlTok::ATagNm)) {
                    break;
                }
                ATokN++;
                if (ATokN >= RefHtmlDoc->GetToks()) {
                    break;
                }
            }
            ...
            forever
            {
                ...
                ATok = RefHtmlDoc->GetTok(ATokN, ATokSym, ATokStr);
                if (THtmlTok::IsBreakTok(ATok)) {
                    break;
                }
                if ((ATokSym == hsyStr) || (ATokSym == hsyNum)) {
                    PrevATokV.Add(ATok);
                }
                if (ATokV.Len() >= HldWnLen) {
                    break;
                }
            }
            // after <A> tags
            ...
            for (int HTagN = 1; HTagN <= 6; HTagN++) {
                HtmlDoc->AddTokV(HTokV[HTagN - 1]);
            }
            ...
            if (TokStr == THtmlTok::TitleTagNm) {
            ...
            else if (THtmlTok::IsHTag(TokStr, HTagN)) {
                ...
                ActHTagN = HTagN;
                {
                    for (int HTagN = ActHTagN; HTagN <= 6; HTagN++) {
                        HTokV[HTagN - 1].Clr();
                    }
                }
...
void TWebPg::GetOutDescUrlStrKdV(TStrKdV& OutDescUrlStrKdV) const
{
    ...
    TStr TokStr;
    int TokN = 0;
    int Toks = HtmlDoc->GetToks();
    while (TokN < Toks) {
        Tok = HtmlDoc->GetTok(TokN, TokSym, TokStr);
        TokN++;
        if ((TokSym == hsyBTag) && (TokStr == THtmlTok::ATagNm)) {
            TStr RelUrlStr;
            if (Tok->IsUrlTok(RelUrlStr)) {
...
bool TWebPg::IsTxt() const
{
    if ((!HttpResp->IsContType()) || HttpResp->IsContType(THttp::TextFldVal)) {
        ...
        while ((ChN < 100) && (ChN < StrLen)) {
            char Ch = Str[ChN++];
            if (((' ' <= Ch) && (Ch <= '~')) || (Ch == TCh::TabCh) || (Ch == TCh::LfCh) ||
                (Ch == TCh::CrCh)) {
                PrintChs++;
            }
        }
