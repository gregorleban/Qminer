...
bool TSysProc::ExeProc(const TStr &ExeFNm, TStr &ParamStr)
{
    ...
    BOOL Ok = CreateProcess(ExeFNm.CStr(), // module name
...
TStr TSysMemStat::GetInfoStr()
{
    TChA ChA;
    ChA += "Memory Load:";
    ChA += TUInt64::GetMegaStr(GetLoad());
    ChA += "\r\n";
    ChA += "Total Physical:";
    ChA += TUInt64::GetMegaStr(GetTotalPhys());
    ChA += "\r\n";
    ChA += "Available Physical:";
    ChA += TUInt64::GetMegaStr(GetAvailPhys());
    ChA += "\r\n";
    ChA += "Total Page File:";
    ChA += TUInt64::GetMegaStr(GetTotalPageFile());
    ChA += "\r\n";
    ChA += "Available Page File:";
    ChA += TUInt64::GetMegaStr(GetAvailPageFile());
    ChA += "\r\n";
    ChA += "Total Virtual:";
    ChA += TUInt64::GetMegaStr(GetTotalVirtual());
    ChA += "\r\n";
    ChA += "Available Virtual:";
    ChA += TUInt64::GetMegaStr(GetAvailVirtual());
    ChA += "\r\n";
    return ChA;
}
...
TStr TSysMemStat::GetStr()
{
    ...
    ChA += TUInt64::GetStr(GetLoad());
    ChA += ' ';
    ChA += TUInt64::GetStr(GetTotalPhys());
    ChA += ' ';
    ChA += TUInt64::GetStr(GetAvailPhys());
    ChA += ' ';
    ChA += TUInt64::GetStr(GetTotalPageFile());
    ChA += ' ';
    ChA += TUInt64::GetStr(GetAvailPageFile());
    ChA += ' ';
    ChA += TUInt64::GetStr(GetTotalVirtual());
    ChA += ' ';
...
TTm TSysTm::GetCurUniTm()
{
    SYSTEMTIME SysTm;
    GetSystemTime(&SysTm);
    return TTm(SysTm.wYear, SysTm.wMonth, SysTm.wDay, SysTm.wDayOfWeek, SysTm.wHour, SysTm.wMinute,
...
TTm TSysTm::GetLocTmFromUniTm(const TTm &Tm)
{
    ...
    // return local-time
    return TTm(LocSysTm.wYear, LocSysTm.wMonth, LocSysTm.wDay, LocSysTm.wDayOfWeek, LocSysTm.wHour,
               LocSysTm.wMinute, LocSysTm.wSecond, LocSysTm.wMilliseconds);
}
...
TTm TSysTm::GetUniTmFromLocTm(const TTm &Tm)
{
    ...
    // return system-time
    return TTm(UniSysTm.wYear, UniSysTm.wMonth, UniSysTm.wDay, UniSysTm.wDayOfWeek, UniSysTm.wHour,
               UniSysTm.wMinute, UniSysTm.wSecond, UniSysTm.wMilliseconds);
}

uint64 TSysTm::GetProcessMSecs()
{
    FILETIME CreationTime, ExitTime, KernelTime, UserTime;
    IAssert(GetProcessTimes(GetCurrentProcess(), &CreationTime, &ExitTime, &KernelTime, &UserTime));
...
TStr TSysStr::GetMsgStr(const DWORD &MsgCd)
{
    ...
    LPVOID lpMsgBuf;
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, MsgCd,
...
{
    HKEY hKey;
    DWORD RetCd = RegOpenKeyEx(BaseKey->GetHandle(), SubKeyNm.CStr(), 0, KEY_ALL_ACCESS, &hKey);
...
void TRegKey::GetKeyNmV(TStrV &KeyNmV) const
{
    ...
    DWORD RetCd = RegQueryInfoKey(hKey,           // key handle
    ...
    if (SubKeys > 0) {
        ...
        forever
        {
            ...
            DWORD RetCd = RegEnumKeyEx(hKey,             // handle of key to enumerate
...
void TRegKey::GetValV(TStrKdV &ValNmStrKdV) const
{
    ...
    if (Vals > 0) {
        ...
        forever
        {
            ...
            DWORD ValType;
            DWORD RetCd = RegEnumValue(hKey,          // handle of key to query
...
TStdIOPipe::TStdIOPipe(const TStr &CmdToExe)
    : ChildStdinRd(NULL), ChildStdinWrDup(NULL), ChildStdoutWr(NULL), ChildStdoutRdDup(NULL)
{
    ...
    EAssert(DuplicateHandle(GetCurrentProcess(), ChildStdoutRd, GetCurrentProcess(),
    ...
    EAssert(DuplicateHandle(GetCurrentProcess(), ChildStdinWr, GetCurrentProcess(),
                            &ChildStdinWrDup, 0, FALSE, DUPLICATE_SAME_ACCESS));
...
{
    if (ChildStdinRd != NULL)
        CloseHandle(ChildStdinRd);
    if (ChildStdinWrDup != NULL)
        CloseHandle(ChildStdinWrDup);
    if (ChildStdoutWr != NULL)
        CloseHandle(ChildStdoutWr);
    if (ChildStdoutRdDup != NULL)
...
TTm TSysTm::GetCurUniTm()
{
    ...
    return TTm(1900 + tms.tm_year, tms.tm_mon + 1, tms.tm_mday, tms.tm_wday, tms.tm_hour,
               tms.tm_min, tms.tm_sec, tv.tv_usec / 1000);
}
...
TTm TSysTm::GetLocTmFromUniTm(const TTm &Tm)
{
    ...
    return TTm(1900 + tmr.tm_year, tmr.tm_mon + 1, tmr.tm_mday, tmr.tm_wday, tmr.tm_hour,
               tmr.tm_min, tmr.tm_sec, MSec);
}
...
bool TSysProc::ExeProc(const TStr &ExeFNm, TStr &ParamStr)
{
    ...
    for (int i = 0; i < SArgV.Len(); i++)
        argv[i + 1] = strdup(SArgV[i].CStr());
...
TSysMemStat::TSysMemStat()
{
    ...
    EAssertR(7 == fscanf(f, "%ld %ld %ld %ld %ld %ld %ld", &result.size, &result.resident,
                         &result.share, &result.text, &result.lib, &result.data, &result.dt),
             "Error parsing /proc/self/statm");
    // close file
    fclose(f);
