...
void TXmlObjSer::AssertXmlHd(const PXmlTok& XmlTok, const TStr& Nm, const TStr& TypeNm)
{
...
TXmlObjSerTagNm::TXmlObjSerTagNm(TSOut& _SOut, const bool& ETagP, const TStr& Nm,
                                 const TStr& TypeNm, const TStr& ArgNm, const TStr& ArgVal)
    : TagNm(), SOut(&_SOut)
{
    if (Nm != "-") {
        SOut->PutCh('<');
...
TXmlObjSerTagNm::TXmlObjSerTagNm(TSOut& _SOut, const bool& ETagP, const TStr& Nm,
                                 const TStr& TypeNm, const TStr& ArgNm1, const TStr& ArgVal1,
                                 const TStr& ArgNm2, const TStr& ArgVal2, const TStr& ArgNm3,
                                 const TStr& ArgVal3, const TStr& ArgNm4, const TStr& ArgVal4)
    : TagNm(), SOut(&_SOut)
{
    if (Nm != "-") {
        SOut->PutCh('<');
...
TXmlChDef::TXmlChDef()
    : Chs(TUCh::Vals), CharChSet(), CombChSet(), ExtChSet(), LetterChSet(), DigitChSet(),
      NameChSet(), PubidChSet(), EntityNmToValH(100)
{
    ...
    SetChTy(CharChSet, 0x1);
    SetChTy(CharChSet, 0x3);
    SetChTy(CharChSet, 0x6);
    SetChTy(CharChSet, 11);
    SetChTy(CharChSet, 24);
    SetChTy(CharChSet, 27);
    // regular characters
    SetChTy(CharChSet, 0x9);
    SetChTy(CharChSet, 0xA);
    SetChTy(CharChSet, 0xD);
    SetChTy(CharChSet, 0x20, TUCh::Mx);
    // BaseChar ::=  [#x0041-#x005A] | [#x0061-#x007A] | [#x00C0-#x00D6] |
    //  [#x00D8-#x00F6] | [#x00F8-#x00FF] | ...
    TBSet BaseChSet(Chs);
    SetChTy(BaseChSet, 0x41, 0x5A);
    SetChTy(BaseChSet, 0x61, 0x7A);
    SetChTy(BaseChSet, 0xC0, 0xD6);
    SetChTy(BaseChSet, 0xD8, 0xF6);
    ...
    NameChSet =
        LetterChSet | DigitChSet | uchar('.') | uchar('-') | uchar('_') | uchar(':') | CombChSet;
    // PubidChar ::=  #x20 | #xD | #xA | [a-zA-Z0-9] | [-'()+,./:=?;!*#@$_%]
    PubidChSet.Gen(Chs);
    SetChTy(PubidChSet, 0x20);
    SetChTy(PubidChSet, 0xD);
    SetChTy(PubidChSet, 0xA);
    SetChTy(PubidChSet, 'a', 'z');
    SetChTy(PubidChSet, 'A', 'Z');
    SetChTy(PubidChSet, '0', '9');
    SetChTy(PubidChSet, "-'()+,./:=?;!*#@$_%");
...
void TXmlLx::ToNrSpacing()
{
    ...
    else if (Spacing == xspPreserve) {
        int SrcChN = 0;
        int DstChN = 0;
        while (SrcChN < TxtChA.Len()) {
            if (TxtChA[SrcChN] == TCh::CrCh) {
                TxtChA.PutCh(DstChN, TCh::LfCh);
                SrcChN++;
                DstChN++;
                if ((SrcChN < TxtChA.Len()) && (TxtChA[SrcChN] == TCh::LfCh)) {
                    SrcChN++;
                }
            }
            else {
    ...
    if (Ch == '#') {
        ...
        if ((!EntityNm.Empty()) && (Ch == ';')) {
            ...
            if (IsEntityNm(EntityNm, EntityVal)) { /*intentionaly empty*/
                ...
                EThrow(TStr("Entity-Reference (") + EntityNm + ") does not exist.");
            }
            return EntityVal;
        }
        else {
            EThrow("Invalid Entity-Reference.");
            Fail;
            return TStr();
        }
    }
...
TStr TXmlLx::GetPEReference()
{
    ...
    TStr EntityNm = GetName();
    if ((EntityNm.Empty()) || (Ch != ';')) {
        EThrow("Invalid PEntity-Reference.");
    }
    GetCh();
    TStr EntityVal;
    if (IsPEntityNm(EntityNm, EntityVal)) { /*intentionaly empty*/
        ...
        EThrow(TStr("PEntity-Reference (") + EntityNm + ") does not exist.");
    }
    return EntityVal;
}
...
void TXmlLx::GetEq()
{
    ...
    if (Ch == '=') {
        ...
        EThrow("Equality ('=') character expected.");
    }
    GetWs(false);
}
...
TStr TXmlLx::GetName(const TStr& RqNm)
{
    ...
    if (Nm == RqNm) {
        ...
        EThrow(TStr("Name '") + RqNm + "' expected.");
        Fail;
        return TStr();
    }
}
...
void TXmlLx::GetComment()
{
    ...
    forever
    {
        GetCh();
        if (!ChDef.IsChar(Ch)) {
            EThrow("Invalid comment character.");
        }
        if (Ch == '-') {
            if (GetCh() == '-') {
                if (GetCh() == '>') {
                ...
                TxtChA += '-';
                TxtChA += Ch; // special case if single '-'
            }
        }
        else {
...
TStr TXmlLx::GetAttValue()
{
    ...
    uchar QCh = Ch;
    if ((QCh != '"') && (QCh != '\'')) {
        EThrow("Invalid attribute-value start.");
    }
    TChA ValChA;
    GetCh();
    forever
...
TStr TXmlLx::GetVersionNum()
{
    ...
    char QCh = Ch;
    if ((Ch != '\'') && (Ch != '"')) {
        EThrow("Quote character (' or \") expected.");
    }
    TChA VerNumChA;
    GetCh();
    do {
...
TStr TXmlLx::GetEncName()
{
    ...
    if ((('a' <= Ch) && (Ch <= 'z')) || (('A' <= Ch) && (Ch <= 'Z'))) {
        ...
        EThrow("Invalid encoding-name character.");
    }
    GetCh();
    while (Ch != QCh) {
        if ((('a' <= Ch) && (Ch <= 'z')) || (('A' <= Ch) && (Ch <= 'Z')) ||
...
TStr TXmlLx::GetStalVal()
{
    ...
    while (Ch != QCh) {
        if (('a' <= Ch) && (Ch <= 'z')) {
            ...
            EThrow("Invalid standalone-value character.");
        }
        GetCh();
    }
    GetCh();
    TStr StalVal = StalChA;
    if ((StalVal == "yes") || (StalVal == "no")) {
        return StalVal;
    }
    else {
        EThrow("Invalid standalone-value.");
        Fail;
        return TStr();
    }
}
...
void TXmlLx::GetXmlDecl()
{
    ...
    // [24] VersionInfo ::=  S 'version' Eq (' VersionNum ' | " VersionNum ")
    GetWs(true);
    TStr VerNm = GetName("version");
    GetEq();
    TStr VerVal = GetVersionNum();
    if (VerVal != "1.0") {
        EThrow("Invalid XML version.");
    }
    AddArg(VerNm, VerVal);
    GetWs(false);
    if (Ch != '?') {
        ...
        //  ('"' EncName '"' |  "'" EncName "'" )
        TStr EncNm = GetName("encoding");
        GetEq();
        TStr EncVal = GetEncName();
        AddArg(EncNm, EncVal);
    }
    GetWs(false);
    if (Ch != '?') {
        ...
        //  (("'" ('yes' | 'no') "'") | ('"' ('yes' | 'no') '"'))
        TStr StalNm = GetName("standalone");
        GetEq();
        TStr StalVal = GetStalVal();
...
TStr TXmlLx::GetSystemLiteral()
{
    ...
    while (Ch != QCh) {
        if (!ChDef.IsChar(Ch)) {
            EThrow("Invalid System-Literal character.");
        }
        LitChA += Ch;
        GetCh();
    }
...
TStr TXmlLx::GetPubidLiteral()
{
    ...
    while (Ch != QCh) {
        if (!ChDef.IsPubid(Ch)) {
            EThrow("Invalid Public-Id-Literal character.");
        }
        LitChA += Ch;
        GetCh();
    }
...
void TXmlLx::GetDocTypeDecl()
{
    ...
    if (Ch == '>') {
        ...
        EThrow("Invalid end-of-tag in document-type-declaration.");
    }
    TagNm = DocTypeDeclNm;
}
...
void TXmlLx::GetElement()
{
    ...
    while (Ch != '>') {
        if (!ChDef.IsChar(Ch)) {
            EThrow("Invalid Element character.");
        }
        TxtChA += Ch;
        GetCh();
    }
...
TStr TXmlLx::GetEntityValue()
{
    ...
    uchar QCh = Ch;
    if ((QCh != '"') && (QCh != '\'')) {
        EThrow("Invalid entity-value start.");
    }
    TChA ValChA;
    GetCh();
    forever
    {
        if (!ChDef.IsChar(Ch)) {
            EThrow("Invalid entity-value character.");
        }
        if (Ch == QCh) {
            GetCh();
            break;
...
void TXmlLx::GetEntity()
{
    ...
    if (Ch == '>') {
        ...
        EThrow("Invalid end-of-tag in entity-declaration.");
    }
    TagNm = EntityNm;
}
...
void TXmlLx::GetCDSect()
{
    ...
    if (Ch == '[') {
        ...
        EThrow("Invalid start of CDATA section.");
    }
    TxtChA.Clr();
    forever
    {
        if (!ChDef.IsChar(Ch)) {
            EThrow("Invalid CDATA character.");
        }
        if ((Ch == '>') && (TxtChA.Len() >= 2) && (TxtChA.LastLastCh() == ']') &&
            (TxtChA.LastCh() == ']')) {
            GetCh();
            TxtChA.Pop();
            TxtChA.Pop();
        ...
        if (Ch == '?') {
            ...
            if (Ch == '[') {
                ...
                if (TagNm == "CDATA") {
                    ...
                    EThrow(TStr("Invalid tag after '<![' (") + TagNm + ").");
                }
            }
            else if (Ch == '-') {
                Sym = xsyComment;
                ...
                else if (TagNm == "ELEMENT") {
                    GetElement();
                    Sym = xsyElement;
                }
                else if (TagNm == "ATTLIST") {
                    GetAttList();
                    Sym = xsyAttList;
                }
                else if (TagNm == "ENTITY") {
                    GetEntity();
                    Sym = xsyEntity;
                }
                else if (TagNm == "NOTATION") {
                    GetNotation();
                    Sym = xsyNotation;
                }
                else {
                    EThrow(TStr("Invalid tag (") + TagNm + ").");
                }
            }
        }
        else if (Ch == '/') {
            ...
            if (Ch == '/') {
                if (GetCh() == '>') {
                    ...
                    EThrow("Invalid Empty-Element-Tag.");
                }
            }
            else {
                Sym = xsySTag;
...
void TXmlLx::EThrow(const TStr& MsgStr) const
{
    ...
    FPosChA += SIn->GetSNm();
    FPosChA += " Line:";
    FPosChA += TInt::GetStr(LnN);
    FPosChA += " Char:";
    FPosChA += TInt::GetStr(LnChN);
    FPosChA += "]";
        ...
        if (Ch() != '&') {
            ...
            if (Ch.GetCh() == '#') {
                ...
                if ((!EntityNm.Empty()) && (Ch() == ';')) {
                    ...
                    else if (EntityNm == "amp") {
                        PlainChA += '&';
                    }
                    else if (EntityNm == "apos") {
                        PlainChA += '\'';
                    }
                    else if (EntityNm == "lt") {
                        PlainChA += '<';
...
bool TXmlTok::GetBoolArgVal(const TStr& ArgNm, const TStr& TrueVal, const bool& DfVal) const
{
    int ArgN = ArgNmValV.SearchForw(TStrKd(ArgNm));
    return (ArgN == -1) ? DfVal : (ArgNmValV[ArgN].Dat == TrueVal);
}

bool TXmlTok::GetBoolArgVal(const TStr& ArgNm, const TStr& TrueVal, const TStr& FalseVal,
                            const bool& DfVal) const
{
    int ArgN = ArgNmValV.SearchForw(TStrKd(ArgNm));
    if (ArgN == -1) {
        return DfVal;
...
int TXmlTok::GetIntArgVal(const TStr& ArgNm, const int& DfVal) const
{
    ...
    if (ArgN == -1) {
        ...
        int Val;
        if (ArgNmValV[ArgN].Dat.IsInt(Val)) {
            return Val;
        }
        else {
            return DfVal;
...
double TXmlTok::GetFltArgVal(const TStr& ArgNm, const double& DfVal) const
{
    ...
    if (ArgN == -1) {
        ...
        double Val;
        if (ArgNmValV[ArgN].Dat.IsFlt(Val)) {
            return Val;
        }
        else {
            return DfVal;
...
PXmlTok TXmlTok::GetTagTok(const TStr& TagPath) const
{
    if (TagPath.Empty()) {
        ...
        TStr TagNm;
        TStr RestTagPath;
        TagPath.SplitOnCh(TagNm, '|', RestTagPath);
        PXmlTok SubTok;
        ...
        if ((SubTok.Empty()) || (RestTagPath.Empty())) {
            return SubTok;
        }
        else {
            return SubTok->GetTagTok(RestTagPath);
        }
    }
}
...
void TXmlTok::GetTagTokV(const TStr& TagPath, TXmlTokV& XmlTokV) const
{
    ...
    TStr PreTagPath;
    TStr TagNm;
    TagPath.SplitOnLastCh(PreTagPath, '|', TagNm);
...
void TXmlTok::AddTokToChA(const bool& XmlP, TChA& ChA) const
{
    ...
    case xsyStr:
        if (XmlP) {
            ChA += TXmlLx::GetXmlStrFromPlainStr(Str);
        }
        else {
            ChA += Str;
        }
    ...
    case xsyTag:
        if (XmlP) {
            ...
            for (int ArgN = 0; ArgN < GetArgs(); ArgN++) {
                ...
                GetArg(ArgN, ArgNm, ArgVal);
                if (XmlP) {
                    ArgVal = TXmlLx::GetXmlStrFromPlainStr(ArgVal);
                }
...
PXmlTok TXmlDoc::LoadTxtElement(TXmlLx& Lx)
{
        ...
        forever
        {
            ...
            if (Lx.Sym == xsyETag) {
                if (Tok->GetStr() == Lx.TagNm) {
                    ...
                    TStr MsgStr = TStr("Invalid End-Tag '") + Lx.TagNm + "' ('" + Tok->GetStr() +
                                  "' expected).";
                    Lx.EThrow(MsgStr);
                }
            }
                ...
                switch (Lx.Sym) {
                case xsySTag:
                    SubTok = LoadTxtElement(Lx);
                    break;
                case xsySETag:
                case xsyStr:
                case xsyQStr:
                case xsyWs:
                    SubTok = TXmlTok::GetTok(Lx);
                    break;
                case xsyPI:
                case xsyComment:
                    break;
                default:
                    Lx.EThrow("Content or End-Tag expected.");
                }
                if (!SubTok.Empty()) {
    ...
    else if (Lx.Sym == xsyETag) {
        TStr MsgStr = TStr("Xml-Element (Start-Tag or Empty-Element-Tag) required.") +
...
PXmlTok TXmlDoc::GetTagTok(const TStr& TagPath) const
{
    if (TagPath.Empty()) {
        ...
        if ((Tok->GetSym() == xsyTag) && (Tok->GetStr() == TagNm)) {
            if (RestTagPath.Empty()) {
                return Tok;
            }
            else {
                return Tok->GetTagTok(RestTagPath);
            }
        }
        else {
            return NULL;
...
bool TXmlDoc::GetTagTokBoolArgVal(const TStr& TagPath, const TStr& ArgNm, const bool& DfVal) const
{
    PXmlTok TagTok;
...
int TXmlDoc::GetTagTokIntArgVal(const TStr& TagPath, const TStr& ArgNm, const int& DfVal) const
{
    PXmlTok TagTok;
    if (IsTagTok(TagPath, TagTok)) {
...
double TXmlDoc::GetTagTokFltArgVal(const TStr& TagPath, const TStr& ArgNm,
                                   const double& DfVal) const
{
    PXmlTok TagTok;
    if (IsTagTok(TagPath, TagTok)) {
...
TStr TXmlDoc::GetTagTokStrArgVal(const TStr& TagPath, const TStr& ArgNm, const TStr& DfVal) const
{
    PXmlTok TagTok;
    if (IsTagTok(TagPath, TagTok)) {
...
bool TXmlDoc::SkipTopTag(const PSIn& SIn)
{
    ...
    try {
        ...
        if (Lx.Sym == xsyXmlDecl) {
            Lx.GetSym();
        }
        LoadTxtMiscStar(Lx);
        if (Lx.Sym == xsyDocTypeDecl) {
            Lx.GetSym();
        }
        LoadTxtMiscStar(Lx);
        Ok = true;
...
PXmlDoc TXmlDoc::LoadTxt(const TStr& FNm, const TXmlSpacing& Spacing)
{
    ...
    return LoadTxt(SIn, Spacing);
}

void TXmlDoc::LoadTxt(const TStr& FNm, TXmlDocV& XmlDocV, const TXmlSpacing& Spacing)
{
    XmlDocV.Clr();
    PSIn SIn = TFIn::New(FNm);
    TXmlLx Lx(SIn, Spacing);
...
TXmlLxSym TXmlParser::GetSym()
{
    ...
    if (Ch == '<') { // load tag
        ...
        if (StrLen > 1 && SymStr[StrLen - 1] == '/') {
            Sym = xsyETag;
            SymStr[StrLen - 1] = 0;
            for (char* c = SymStr.CStr() + StrLen - 2; TCh::IsWs(*c); c--) {
                *c = 0;
            }
        }
    }
...
{
    EAssertR(GetTag(TagStr) == xsySTag,
             TStr::Fmt("Expected '<%s>'. Found '%s'", TagStr.CStr(), SymStr.CStr()).CStr());
    EAssertR(GetSym(TagVal) == xsyStr, "Expected string tag.");
    EAssertR(GetTag(TagStr) == xsyETag,
             TStr::Fmt("Expected '</%s>'. Found '%s'", TagStr.CStr(), SymStr.CStr()).CStr());
}
...
TXmlLxSym TXmlParser::GetTag(const TChA& TagStr)
{
    ...
    EAssertR(
        TagStr == SymStr,
        TStr::Fmt("Expected xml symbol '%s'. Found '%s'", TagStr.CStr(), SymStr.CStr()).CStr());
    return Sym;
}
