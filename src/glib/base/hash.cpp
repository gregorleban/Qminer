...
TBigStrPool::TBigStrPool(TSize MxBfLen, uint _GrowBy)
...
{
    SOut.Save(uint64(MxBfL));
    SOut.Save(uint64(BfL));
    SOut.Save(GrowBy);
    if (BfL > 0) {
        SOut.SaveBf(Bf, BfL);
    }
...
// Adds Len characters to pool. To append a null terminated string Len must be equal to strlen(s) +
// 1
int TBigStrPool::AddStr(const char* Str, uint Len)
{
    IAssertR(Len > 0,
             "String too short (lenght includes the null character)"); // J: if (! Len) return -1;
    Assert(Str);
    Assert(Len > 0);
    if (Len == 1 && IdOffV.Len() > 0) {
        return 0;
    } // empty string
    if (BfL + Len > MxBfL) {
        Resize(BfL + Len);
